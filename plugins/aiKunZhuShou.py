...
config = form({
    's_ikuu_qlname': form.string().title('设置对接容器').default('').description('青龙容器参数用丨分割'),
    's_ikuu_osname': form.string().title('青龙变量名').default('').description('青龙容器内的变量名'),
    's_ikuu_notify': form.string().title('通知渠道').default('').description('检测通知推送渠道'),
    's_ikuu_proxy_api': form.string().title('代理API地址').default('').description('代理API返回txt格式ip:port，未配置则直连'),
})
_CONFIG_FIELD_MAP = {
