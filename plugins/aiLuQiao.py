...
config = plugin.Form({
    ...
    's_alq_qlname': plugin.Form.string().title('设置对接容器').default('').description('面板容器参数；use_daipanel=true 时此处应填写呆呆面板 Host丨AppKey丨AppSecret，不填则回退使用 默认配置 全局配置'),
    's_alq_use_daipanel': plugin.Form.boolean().title('使用呆呆面板').default(False).description('勾选后使用呆呆面板，不勾选使用青龙面板'),
    's_alq_panel_group': plugin.Form.string().title('呆呆面板分组').default('').description('仅 use_daipanel=true 时生效，填写后同步写入 group 字段'),
    's_alq_osname': plugin.Form.string().title('青龙变量名').default('S_ALQ').description('上传到面板的变量名'),
    's_alq_notify': plugin.Form.string().title('通知渠道').default('').description('检测通知推送渠道'),
})
_CONFIG_FIELD_MAP = {
...
def bind_account():
    ...
    auth_time = str('2099-12-31').strip()
