...
config = plugin.Form({
    ...
    'dd_axj_dd_axj_qlname': plugin.Form.string().title('对接系统配置').default('').description('必填项，青龙面板对接信息'),
    'dd_axj_dd_axj_osname': plugin.Form.string().title('系统变量名').default('').description('系统容器内变量名'),
    'dd_axj_enable_proxy': plugin.Form.boolean().title('是否启用代理').default(False).description('是否启用代理功能'),
    'dd_axj_proxy_pool_url': plugin.Form.string().title('代理池地址').default('').description('代理API服务地址'),
    'dd_axj_enable_remark': plugin.Form.boolean().title('启用备注功能').default(False).description('是否启用账号备注功能'),
    'dd_axj_enable_sms': plugin.Form.boolean().title('开启短信登录').default(False).description('是否允许使用短信登录(关闭后只能抓包)'),
})
_CONFIG_FIELD_MAP = {
