...
# [disable: false]
# [admin: false]
# [rule: ^武义登陆$|^武义登录$|^武义签到$|^武义管理$|^武义超管$|^武义配置$|^wyfk$]
# [cron: 12 7 * * *]
...
class QL:
    def __init__(self, ql=None):
        ql = ql or {}
        self.host = str(ql.get("host") or ql.get("address") or "").rstrip("/")
        self.client_id = str(ql.get("client_id") or ql.get("clientId") or "")
        self.client_secret = str(ql.get("client_secret") or ql.get("clientSecret") or "")
        self.token = self._get_token()
    def _get_token(self):
        if not (self.host and self.client_id and self.client_secret): return ""
        try:
            qs = requests.compat.urlencode({"client_id": self.client_id, "client_secret": self.client_secret})
            data = requests.get(f"{self.host}/open/auth/token?{qs}", timeout=15).json().get("data") or {}
            return data.get("token") or ""
        except Exception: return ""
    def _request(self, method, path, body=None):
        if not self.token: return {"code": -1, "message": "青龙认证失败", "data": None}
        path = "/" + str(path or "").lstrip("/")
        if not path.startswith("/open/"): path = "/open" + path
        try: return requests.request(method, self.host + path, headers={"Authorization": "Bearer " + self.token}, json=body, timeout=20).json()
        except Exception as e: return {"code": -1, "message": str(e), "data": None}
    def envGet(self, name=""):
        data = self._request("GET", "/envs").get("data") or []
        return [x for x in data if x.get("name") == name] if name and isinstance(data, list) else data
    def envSet(self, data):
        try: body = json.loads(data) if isinstance(data, str) else data
        except Exception: body = data
        return self._request("POST", "/envs", body)
    def envUpdate(self, data):
        try: body = json.loads(data) if isinstance(data, str) else data
        except Exception: body = data
        return self._request("PUT", "/envs", body)
class TOOL:
    def __init__(self, sender, plugin=None):
        plugin = plugin or {}; self.sender = sender; self.plugin_pre = str(plugin.get("key") or "plugin_"); self.plugin_name = str(plugin.get("name") or ""); self.plugin_ver = str(plugin.get("ver") or "")
        self.msg = str(sender.getMessage() or ""); self.userId = str(sender.getUserID() or ""); self.chatId = str(sender.getChatID() or ""); self.imType = str(sender.getImtype() or ""); self.isAdmin = bool(sender.isAdmin())
    def bucketGet(self, bucket, key, default=""): return sg.bucketGet(bucket, key, default)
    def bucketSet(self, bucket, key, value=""): return sg.bucketSet(bucket, key, value)
    def bucketAllKeys(self, bucket): return sg.bucketAllKeys(bucket)
    def replyMsg(self, msg): return self.sender.reply(str(msg))
    def pushMsg(self, user_id=None, chat_id=None, im_type=None, title="", msg=""): return sg.push(im_type or self.imType, chat_id or "", user_id or "", title or "", msg or title or "")
    def log_info(self, msg): print(str(msg))
    def log_err(self, msg): print(str(msg))
    def hide_phone_number(self, value):
        value = str(value or "")
        return value if len(value) <= 7 else value[:3] + "****" + value[-4:]
    def platformArr(self): return ["qq", "wx", "tg", "web", "fake"]
    def user_can_add(self): return True
    def update_user_balance(self): return True
    def account_manger_admin(self): return self.replyMsg("账号管理")
...
class ACCOUNT:
    ...
    def client_qinglong(self):
        if not self.conf['host'] or not self.conf['client_id'] or not self.conf['client_secret']:
            tool.replyMsg("青龙配置有误，请检查配置正确配置！")
            exit()
        ql={
            "name":"ql",
            "host":self.conf['host'],
...
-----END PUBLIC KEY-----"""
        ...
        self.conf = {
            ...
            "ocr_host":tool.bucketGet("vhook_common", "ocr_host") or "",
    ...
    def config(self):
        res = self.common_get("/api/article/channel_list?channel_id=657a5b3d79f6be03b8fd7fb1&isDiFangHao=false&is_new=true&list_count=0&size=20")
        if res:
    ...
    def task_list(self):
        ...
        for index,task in enumerate(res['data']['list'],start=1):
            ...
            if task['name'] == '分享资讯给好友':
                shareFinish = False
            if task['name'] == "使用本地服务":
                doTask_res = self.common_post("/api/user_mumber/doTask",{"memberType":6,"member_type":6})
