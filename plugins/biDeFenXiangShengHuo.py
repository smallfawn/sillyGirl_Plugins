...
# [description: 插件自带任务! 使用前请先配参；用户指令: 粉象上车,粉象管理,粉象查询,粉象教程；运行在定时推送里面设置自动运行时间；建议每天21点前运行一次，22点左右运行一次!相关配置桶: bd_fxconfig 用户数据桶: bd_fxcks]
# [depe: ["requests"]]
...
config = form({
    'bd_fxconfig_yqurl': form.string().title('邀请连接').default('').description('app-我的-上面邀好友-复制链接 不设置默认是作者的哦！'),
    'bd_fxconfig_notify': form.string().title('管理员通知').default('').description('设置接受管理员通知的渠道，如 qq,wx,tg  用英文“,”符号分割,不设置不推送'),
    'bd_fxconfig_sdyx': form.boolean().title('手动运行').default(False).description('是否允许用户手动执行任务(默认否)'),
})
_CONFIG_FIELD_MAP = {
    ('bd_fxconfig', 'yqurl'): 'bd_fxconfig_yqurl',
    ('bd_fxconfig', 'notify'): 'bd_fxconfig_notify',
    ('bd_fxconfig', 'sdyx'): 'bd_fxconfig_sdyx',
}
...
class ATM_FX:
        ...
        if self.phone == 'q' or self.phone == 'Q':
            ...
            if self.post_yzm():
                ...
                if code == 'q' or code == 'Q':
                    ...
                    self.code = code
                    if self.get_token():
                        ts = sg.bucketGet('bd_fxcks', self.user)
                        if not ts:
                            data = {
                                f'{self.usid}': {
                                    'name': self.name,
                            ...
                            sg.bucketSet('bd_fxcks', self.user, f'{data}')
                            ...
                            if self.usid in ts:
                                for k, y in ts.items():
                                    if self.usid == k:
                                        ...
                                        sg.bucketSet('bd_fxcks', self.user, f'{ts}')
                                        self.sender.reply(
    ...
    def gl_zh(self):
        ...
        if zh == 'q' or zh == 'Q':
            ...
            sdyx = sg.bucketGet('bd_fxconfig', 'sdyx')
            if sdyx == '':
                sdyx = 'false'
            if sdyx == 'false':
    ...
    def dssq(self):
        ...
        try:
            ...
            if status == "True" or status or status == "true":
                ...
                wxzsm = sg.bucketGet('bd_fxconfig', 'wxzsm')
                sqsj = '2099-12-31'
                sqje = '2099-12-31'
                if sqsj == '':
                    sqsj = 30
                    ...
                    if waitPay == 'q':
                        ...
                        for k, y in ts.items():
                            if self.usid == k:
                                ...
                                self.sender.reply(msg)
                                notify = sg.bucketGet('bd_fxconfig', 'notify')
                                if notify == '':
                                    pass
                                else:
                                    tsqd = notify.split(',')
    ...
    def fxyx(self):
        ts = sg.bucketAllKeys('bd_fxcks')
        start_zhs = {}
        kong = 0
        wsqzhs = {}
        for i in ts:
            ts_data = sg.bucketGet('bd_fxcks', f'{i}')
            ts_data = _sg_literal(ts_data)
            if ts_data == {}:
                kong += 1
                sg.bucketDel('bd_fxcks', f'{i}')
                continue
            else:
        ...
        for k, y in wsqzhs.items():
            ts_data = sg.bucketGet('bd_fxcks', f'{k}')
            ts_data = _sg_literal(ts_data)
            del ts_data[f'{y}']
            sg.bucketSet('bd_fxcks', f'{k}', f'{ts_data}')
    ...
    def fxpz(self):
        yqurl = sg.bucketGet('bd_fxconfig', 'yqurl')
        if yqurl == '':
            pz1 = '未配置'
        else:
            pz1 = '已配置'
        ...
        if zh == 'q' or zh == 'Q':
            ...
            if pz == 'q' or pz == 'Q':
                ...
                sg.bucketSet('bd_fxconfig', 'yqurl', f'{pz}')
            ...
            if pz == 'q' or pz == 'Q':
                ...
                sg.bucketSet('bd_fxconfig', 'sdyx', f'{pz}')
            ...
            if pz == 'q' or pz == 'Q':
                ...
                sg.bucketSet('bd_fxconfig', 'notify', f'{pz}')
            ...
            if pz == 'q' or pz == 'Q':
                ...
                self.sender.replyImage(pz)
                sg.bucketSet('bd_fxconfig', 'wxzsm', f'{pz}')
            ...
            for i in ts:
                ...
                if ts_data == {}:
                    ...
                    for k, y in ts_data.items():
                        ...
                        sg.bucketSet('bd_fxcks', f'{i}', f'{ts_data}')
    ...
    def zdsq(self):
        ...
        if myuid == 'q' or myuid == 'Q':
            ...
            ts = sg.bucketGet('bd_fxcks', myuid)
            if ts == '' or ts == '{}':
                ...
                if xz == 'q' or xz == 'Q':
                    ...
                    if sjts == 'q' or sjts == 'Q':
                        ...
                        if self.sqsj > dqsj:
                            ...
                            new_sqsj = sj + timedelta(days=int(sjts))
                            new_sqsj = new_sqsj.strftime("%Y-%m-%d")
                        ts = sg.bucketGet('bd_fxcks', f'{myuid}')
                        ts = _sg_literal(ts)
                        for k, y in ts.items():
                            if self.usid == k:
                                ts[f'{k}'] = {
                                ...
                                sg.bucketSet('bd_fxcks', myuid, f'{ts}')
                                msg = f'========粉象授权========\n当前用户: {myuid}\n授权用户: {self.name}\n授权id: {self.usid}\n授权天数: {int(sjts)}天\n到期时间: {new_sqsj}'
                                self.sender.reply(msg)
                                break
                            else:
...
if __name__ == '__main__':
    ...
    cjname = '彼得_粉象生活'
    version = 'V6.25'
    sg.bucketSet('bd_cjbb', f'{cjname}', f'{version}')
    if message == '粉象上车':
        atm_fx.fxsc()
    elif message == '粉象管理':
        atm_fx.fxgl()
