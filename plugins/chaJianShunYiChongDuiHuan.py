...
# [disable: false]
# [admin: false]
# [rule: ^顺易充(时间|删除|修正|总结|库存|库存兑换|代理代理|代理配置|代理查询)$]
...
def get_config():
    try:
        ...
        zsm = sg.bucketGet(bucket="G_SYC", key="zsm") or ""
        points_per_month_str = (
            sg.bucketGet(bucket="G_SYC", key="points_per_month") or "100"
        )
        points_per_month = (
        ...
        agent_price_str = (
            sg.bucketGet(bucket="G_SYC", key="agent_price") or "8.88"
        )
        agent_price = (
        ...
        agent_points_str = (
            sg.bucketGet(bucket="G_SYC", key="agent_points_per_month") or "800"
        )
...
def get_user_by_phone(phone):
    try:
        ...
        for user_id in all_users:
            phones_json = sg.bucketGet("G_SYC_user", user_id) or "[]"
...
def delete_expired_accounts():
    ...
    for acc in expired_list:
        ...
        masked_phone = acc["masked_phone"]
        try:
            sg.bucketDel("G_SYC_TOKEN", phone)
            ...
            if user_id:
                ...
                if phone in phones:
                    phones.remove(phone)
                    if phones:
                        sg.bucketSet("G_SYC_user", user_id, json.dumps(phones))
                    else:
                        sg.bucketDel("G_SYC_user", user_id)
            success_count += 1
...
def delete_user_accounts():
    ...
    for user in valid_users:
        ...
        for phone in phones:
            try:
                ...
                try:
                    ...
                    pass
                try:
                    sg.bucketDel("G_SYC_token_status", phone)
                except:
                    pass
                user_success += 1
                success_count += 1
        ...
        try:
            ...
            pass
        try:
            sg.bucketDel("G_SYC_sign", target_user_id)
        except:
            pass
        try:
            sg.bucketDel("G_SYC_coin", target_user_id)
        except:
            pass
        uid_short = (
