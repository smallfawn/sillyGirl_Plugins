...
config = plugin.Form({
    ...
    's_dc_qlname': plugin.Form.string().title('设置对接容器').default('').description('面板容器参数，不填则使用默认配置'),
    's_dc_use_daipanel': plugin.Form.boolean().title('使用呆呆面板').default(False).description('勾选使用呆呆面板，不勾选使用青龙面板'),
    's_dc_panel_group': plugin.Form.string().title('呆呆面板分组').default('').description('填写后新增/更新变量时同步写入group字段，留空则不处理'),
    's_dc_osname': plugin.Form.string().title('提交到青龙的变量名').default('').description('青龙容器内大潮的变量名'),
    's_dc_notify': plugin.Form.string().title('通知渠道').default('').description('配置检测通知推送渠道'),
})
_CONFIG_FIELD_MAP = {
