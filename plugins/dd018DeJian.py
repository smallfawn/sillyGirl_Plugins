...
_CONFIG_FIELD_MAP = {
    ...
    ('dd_dejian_config', 'ql_env_name'): 'dd_dejian_config_ql_env_name',
    ('dd_dejian_config', 'sms_wait_timeout'): 'dd_dejian_config_sms_wait_timeout',
    ('dd_dejian_config', 'admin_ids'): 'dd_dejian_config_admin_ids',
}
...
def config_read_key(key, default=""):
    ...
    if value not in (None, ""):
        return value
    if sg is not None and hasattr(sg, "getParam"):
        try:
            value = sg.getParam(dotted)
            if value not in (None, ""):
                return value
        except Exception:
            pass
        ...
        try:
            ...
            if paid:
                try:
                    ...
                    failed.append({"remark": account_display(account), "reason": "%s；退款失败，请检查配置" % short_error(exc)})
                    continue
