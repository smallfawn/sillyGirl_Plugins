...
# [description: 芳华未来代挂提交；2. 用户提交 手机号#密码；4.]
# [depe: ["requests"]]
...
config = plugin.Form({
    ...
    'fang_hua_wei_lai_panel_type': plugin.Form.string().title('对接面板类型').default('').description('qinglong=青龙面板 daidai=呆呆面板'),
    'fang_hua_wei_lai_fang_hua_wei_lai_qlname': plugin.Form.string().title('对接系统配置').default('').description('青龙:URL丨ID丨Secret 呆呆:URL丨Key丨Secret'),
    'fang_hua_wei_lai_fang_hua_wei_lai_osname': plugin.Form.string().title('系统变量名').default('').description('系统容器内变量名(默认为fhb)'),
    'fang_hua_wei_lai_enable_proxy': plugin.Form.boolean().title('启用代理').default(False).description('是否启用代理功能'),
    'fang_hua_wei_lai_proxy_pool_url': plugin.Form.string().title('代理池地址').default('').description('代理API服务地址'),
    'fang_hua_wei_lai_enable_remark': plugin.Form.boolean().title('启用备注功能').default(False).description('是否启用账号备注功能'),
})
_CONFIG_FIELD_MAP = {
    ('fang_hua_wei_lai', 'panel_type'): 'fang_hua_wei_lai_panel_type',
    ('fang_hua_wei_lai', 'fang_hua_wei_lai_qlname'): 'fang_hua_wei_lai_fang_hua_wei_lai_qlname',
    ('fang_hua_wei_lai', 'fang_hua_wei_lai_osname'): 'fang_hua_wei_lai_fang_hua_wei_lai_osname',
    ('fang_hua_wei_lai', 'enable_proxy'): 'fang_hua_wei_lai_enable_proxy',
    ('fang_hua_wei_lai', 'proxy_pool_url'): 'fang_hua_wei_lai_proxy_pool_url',
    ('fang_hua_wei_lai', 'enable_remark'): 'fang_hua_wei_lai_enable_remark',
}
...
try:
    sg.bucketSet(bucket='fang_hua_wei_lai_sender', key=userid, value=str(senderID))
    sg.bucketSet(bucket='fang_hua_wei_lai_imtype', key=userid, value=str(sender.getImtype()))
except:
    pass

def getusercontent():
    panel_type = sg.bucketGet('fang_hua_wei_lai', 'panel_type') or 'qinglong'
    panel_type = panel_type.lower()

    env_qlconfig = sg.bucketGet('fang_hua_wei_lai', 'fang_hua_wei_lai_qlname') or ''
    env_name = sg.bucketGet('fang_hua_wei_lai', 'fang_hua_wei_lai_osname') or 'fhb'

    if not env_qlconfig:
        sender.reply("❌ 配置错误：请在插件配置中填写【对接系统配置】(面板信息)。")
        exit(0)

    fang_hua_wei_lai_managecommand = sg.bucketGet('fang_hua_wei_lai', 'fang_hua_wei_lai_managecommand') or '芳华未来管理'
    fang_hua_wei_lai_querycommand = sg.bucketGet('fang_hua_wei_lai', 'fang_hua_wei_lai_querycommand') or '芳华未来查询'
    fang_hua_wei_lai_signcommand = sg.bucketGet('fang_hua_wei_lai', 'fang_hua_wei_lai_signcommand') or '芳华未来登录'

    enable_proxy = (sg.bucketGet('fang_hua_wei_lai', 'enable_proxy') or 'false').lower() == 'true'
    proxy_pool_url = sg.bucketGet('fang_hua_wei_lai', 'proxy_pool_url') or ''
    points_bucket = sg.bucketGet('fang_hua_wei_lai', 'points_bucket') or 'dd_sign_points'
    enable_remark = (sg.bucketGet('fang_hua_wei_lai', 'enable_remark') or 'false').lower() == 'true'
    ...
    zsVipmoney = Decimal(sg.bucketGet('fang_hua_wei_lai', 'zsVipmoney') or '0')
    zscoin = int(sg.bucketGet('fang_hua_wei_lai', 'zscoin') or '0')
    reminder_days = int(sg.bucketGet('fang_hua_wei_lai', 'reminder_days') or '2')

    enable_zsm = (sg.bucketGet('fang_hua_wei_lai', 'enable_zsm') or 'false').lower() == 'true'
    zsm = sg.bucketGet('fang_hua_wei_lai', 'zsm') or ''
...
def get_owner_user_id(account, fallback_userid=None):
    ...
    try:
        ...
        pass
    try:
        for owner in sg.bucketAllKeys(bucket='fang_hua_wei_lai_user'):
            try:
...
def send_daily_admin_report(report_data, force_send=False, notify_status=False):
    ...
    if not force_send and sg.bucketGet('fang_hua_wei_lai_runtime', report_key):
        if notify_status:
    ...
    if framework_sent:
        try:
            sg.bucketSet('fang_hua_wei_lai_runtime', report_key, "framework")
        except Exception:
            pass
        if notify_status:
...
def send_user_notice(user_id, msg, title="芳华未来通知"):
    try:
        imtype = sg.bucketGet(bucket='fang_hua_wei_lai_imtype', key=str(user_id)) or sender.getImtype()
    except:
    ...
    targets = []
    try:
        saved_sender = sg.bucketGet(bucket='fang_hua_wei_lai_sender', key=str(user_id))
        if saved_sender:
...
class RemarkManager:
    ...
    def get_account_remark(user_id, account_id):
        try:
            remark_data = sg.bucketGet(bucket='fang_hua_wei_lai_remarks', key=f'{user_id}_{account_id}')
    ...
    def set_account_remark(user_id, account_id, remark):
        try:
            ...
            if remark_clean:
                sg.bucketSet(bucket='fang_hua_wei_lai_remarks', key=f'{user_id}_{account_id}', value=remark_clean)
                return remark_clean
            return ""
        except: return ""
    ...
    def delete_account_remark(user_id, account_id):
        try:
            sg.bucketDel(bucket='fang_hua_wei_lai_remarks', key=f'{user_id}_{account_id}')
            return True
        except: return False
...
class AccountManager:
    ...
    def get_accounts(user_id):
        try:
            value = sg.bucketGet(bucket='fang_hua_wei_lai_user', key=str(user_id))
            if not value: return []
    ...
    def add_account(user_id, account):
        try:
            ...
            if account not in accounts:
                ...
                sg.bucketSet(bucket='fang_hua_wei_lai_user', key=str(user_id), value=str(accounts))
                return True
            return False
        except: return False
        ...
        try:
            ...
            if account in accounts:
                ...
                if accounts:
                    ...
                    sg.bucketDel(bucket='fang_hua_wei_lai_user', key=str(user_id))
                return True
            return False
        except: return False
    ...
    def update_account_token(account, token):
        try:
            ...
            sg.bucketSet(bucket='fang_hua_wei_lai_token', key=str(account), value=encrypted_token)
            return True
        except: return False
    ...
    def update_account_mobile(account, mobile):
        try:
            mobile = str(mobile or "").strip()
            if mobile:
                sg.bucketSet(bucket='fang_hua_wei_lai_mobile', key=str(account), value=mobile)
                return True
            return False
        except: return False
    ...
    def get_account_mobile(account):
        try:
            return sg.bucketGet(bucket='fang_hua_wei_lai_mobile', key=str(account)) or ""
        except: return ""
    ...
    def update_account_web_token(account, web_token):
        try:
            ...
            if web_token:
                sg.bucketSet(bucket='fang_hua_wei_lai_web_token', key=str(account), value=encrypt_token(web_token))
                return True
            return False
        except: return False
    ...
    def get_account_web_token(account):
        try:
            enc = sg.bucketGet(bucket='fang_hua_wei_lai_web_token', key=str(account))
    ...
    def get_token(account):
        try:
            enc = sg.bucketGet(bucket='fang_hua_wei_lai_token', key=str(account))
    ...
    def get_all_users():
        try:
            users = sg.bucketAllKeys(bucket='fang_hua_wei_lai_user')
            user_list = []
            for user in users:
    ...
    def migrate_account(user_id, old_account, new_account, new_token, remark=""):
        try:
            ...
            old_bind_date = sg.bucketGet(bucket='fang_hua_wei_lai_bind_date', key=old_account)
            if old_bind_date and not sg.bucketGet(bucket='fang_hua_wei_lai_bind_date', key=new_account):
                sg.bucketSet(bucket='fang_hua_wei_lai_bind_date', key=new_account, value=old_bind_date)
            ...
            sg.bucketSet(bucket='fang_hua_wei_lai_user', key=str(user_id), value=str(new_accounts))

            AccountManager.update_account_token(new_account, new_token)
            try: sg.bucketDel(bucket='fang_hua_wei_lai_token', key=old_account)
            except: pass
            try: sg.bucketDel(bucket='fang_hua_wei_lai_mobile', key=old_account)
            except: pass
            try: sg.bucketDel(bucket='fang_hua_wei_lai_web_token', key=old_account)
            except: pass
            try:
                pass
            except: pass
...
def process_account_binding(full_token, unique_id, nickname, remark="", aliases=None, acc_type="", legacy_key="", mobile="", web_token="", silent=False):
    try:
        ...
        if is_new:
            try: sg.bucketSet(bucket='fang_hua_wei_lai_bind_date', key=account, value=str(datetime.now().date()))
            except: pass
    ...
    if get_user_input() == "确认删除":
        ...
        for account in accounts:
            try:
                 ...
                 try: sg.bucketDel(bucket='fang_hua_wei_lai_token', key=account)
                 except: pass
                 try: sg.bucketDel(bucket='fang_hua_wei_lai_mobile', key=account)
                 except: pass
                 try: sg.bucketDel(bucket='fang_hua_wei_lai_web_token', key=account)
                 except: pass
                 try:
                     pass
                 except: pass
                 ...
                 for d in range(config['reminder_days'] + 1):
                     ...
                     try: sg.bucketDel('fang_hua_wei_lai_remind_log', remind_key)
                     except: pass
            except: pass
        sender.reply("✅ 批量删除完成")
            ...
            for account_item in context["accounts"]:
                ...
                if days_diff >= 0:
                    ...
                    if is_ck_valid and 0 <= days_diff <= reminder_days_cfg:
                        ...
                        has_reminded = sg.bucketGet('fang_hua_wei_lai_remind_log', remind_key)
...
=================="""
                            if safe_send_message(user, msg, f"到期提醒 {user}-{account}"):
                                try: sg.bucketSet('fang_hua_wei_lai_remind_log', remind_key, "1")
                                except: pass
                                reminded_count += 1
                    continue
...
=================="""
            ...
            if user_has_change:
                if valid_accounts:
                    try: sg.bucketSet(bucket='fang_hua_wei_lai_user', key=str(user), value=str(valid_accounts))
                    except: pass
                else:
                    try: sg.bucketDel(bucket='fang_hua_wei_lai_user', key=str(user))
                    except: pass
