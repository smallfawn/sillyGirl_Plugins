...
const config = new plugin.Form({
  ...
  task_keyword: plugin.Form.string().title("青龙任务关键词").default("福田下单"),
});

async function main() {
  try {
    const cfg = normalize(await config.get());
    if (!cfg.enable) return s.reply("福田下单插件未启用");
    ...
    return s.reply(["福田下单任务已触发", ...tasks.map((task) => `• ${task.name || task.command || task.id}`)].join("\n"));
  } catch (error) {
    return s.reply(`福田下单执行失败：${message(error)}`);
  }
}
...
function normalize(raw) {
  const value = raw || {};
  return { enable: value.enable !== false, qinglongId: Number(value.qinglong_id) || 1, taskKeyword: String(value.task_keyword || "福田下单").trim() || "福田下单" };
}
