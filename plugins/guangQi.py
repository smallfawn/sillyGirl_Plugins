...
# [disable: false]
# [admin: false]
# [rule: ^(广汽)(登录|登陆)$|^登(录|陆)(广汽)$|^(广汽)(查询|管理)$|^(查询|管理)(广汽)$|^广汽清理$|^广汽$|^广汽教程$|^广汽通知 ?(.*)$|^清理广汽$|^广汽广播 ?(.*)$]
# [cron: 5 11 * * *]
# [icon: https://api.iconify.design/lucide:bot.svg]
# [description: 广汽代挂提交；2. 用户提交 手机号#AT#RT，环境变量只写 AT#RT，手机号写入备注并脱敏展示；4.]
# [depe: ["requests"]]
...
config = plugin.Form({
    ...
    'gacmotor_panel_type': plugin.Form.string().title('对接面板类型').default('').description('qinglong=青龙面板 daidai=呆呆面板'),
    'gacmotor_gacmotor_qlname': plugin.Form.string().title('对接系统配置').default('').description('青龙:URL丨ID丨Secret 呆呆:URL丨Key丨Secret'),
    'gacmotor_gacmotor_osname': plugin.Form.string().title('系统变量名').default('').description('系统容器内变量名(默认为GacMotor)'),
    'gacmotor_enable_proxy': plugin.Form.boolean().title('启用代理').default(False).description('是否启用代理功能'),
    'gacmotor_proxy_pool_url': plugin.Form.string().title('代理池地址').default('').description('代理API服务地址'),
    'gacmotor_enable_remark': plugin.Form.boolean().title('启用备注功能').default(False).description('是否启用账号备注功能'),
})
_CONFIG_FIELD_MAP = {
    ('gacmotor', 'panel_type'): 'gacmotor_panel_type',
    ('gacmotor', 'gacmotor_qlname'): 'gacmotor_gacmotor_qlname',
    ('gacmotor', 'gacmotor_osname'): 'gacmotor_gacmotor_osname',
    ('gacmotor', 'enable_proxy'): 'gacmotor_enable_proxy',
    ('gacmotor', 'proxy_pool_url'): 'gacmotor_proxy_pool_url',
    ('gacmotor', 'enable_remark'): 'gacmotor_enable_remark',
}
...
try:
    sg.bucketSet(bucket='gacmotor_sender', key=userid, value=str(senderID))
    sg.bucketSet(bucket='gacmotor_imtype', key=userid, value=str(sender.getImtype()))
except:
    pass

def getusercontent():
    panel_type = sg.bucketGet('gacmotor', 'panel_type') or 'qinglong'
    panel_type = panel_type.lower()

    env_qlconfig = sg.bucketGet('gacmotor', 'gacmotor_qlname') or ''
    env_name = sg.bucketGet('gacmotor', 'gacmotor_osname') or 'GacMotor'

    if not env_qlconfig:
        sender.reply("❌ 配置错误：请在插件配置中填写【对接系统配置】(面板信息)。")
        exit(0)

    gacmotor_managecommand = sg.bucketGet('gacmotor', 'gacmotor_managecommand') or '广汽管理'
    gacmotor_querycommand = sg.bucketGet('gacmotor', 'gacmotor_querycommand') or '广汽查询'
    gacmotor_signcommand = sg.bucketGet('gacmotor', 'gacmotor_signcommand') or '广汽登录'

    enable_proxy = (sg.bucketGet('gacmotor', 'enable_proxy') or 'false').lower() == 'true'
    proxy_pool_url = sg.bucketGet('gacmotor', 'proxy_pool_url') or ''
    points_bucket = sg.bucketGet('gacmotor', 'points_bucket') or 'dd_sign_points'
    enable_remark = (sg.bucketGet('gacmotor', 'enable_remark') or 'false').lower() == 'true'

    randommanagecommand = gacmotor_managecommand
    randomquerycommand = gacmotor_querycommand
    randomsigncommand = gacmotor_signcommand

    zsVipmoney = Decimal(sg.bucketGet('gacmotor', 'zsVipmoney') or '0')
    zscoin = int(sg.bucketGet('gacmotor', 'zscoin') or '0')
    reminder_days = int(sg.bucketGet('gacmotor', 'reminder_days') or '2')

    enable_zsm = (sg.bucketGet('gacmotor', 'enable_zsm') or 'false').lower() == 'true'
    zsm = sg.bucketGet('gacmotor', 'zsm') or ''
...
def get_owner_user_id(account, fallback_userid=None):
    ...
    try:
        ...
        pass
    try:
        for owner in sg.bucketAllKeys(bucket='gacmotor_user'):
            try:
...
def send_daily_admin_report(report_data, force_send=False, notify_status=False):
    ...
    if not force_send and sg.bucketGet('gacmotor_runtime', report_key):
        if notify_status:
    ...
    if framework_sent:
        try:
            sg.bucketSet('gacmotor_runtime', report_key, "framework")
        except Exception:
            pass
        if notify_status:
...
def send_user_notice(user_id, msg, title="广汽通知"):
    try:
        imtype = sg.bucketGet(bucket='gacmotor_imtype', key=str(user_id)) or sender.getImtype()
    except:
    ...
    targets = []
    try:
        saved_sender = sg.bucketGet(bucket='gacmotor_sender', key=str(user_id))
        if saved_sender:
...
class RemarkManager:
    ...
    def get_account_remark(user_id, account_id):
        try:
            remark_data = sg.bucketGet(bucket='gacmotor_remarks', key=f'{user_id}_{account_id}')
    ...
    def set_account_remark(user_id, account_id, remark):
        try:
            ...
            if remark_clean:
                sg.bucketSet(bucket='gacmotor_remarks', key=f'{user_id}_{account_id}', value=remark_clean)
                return remark_clean
            return ""
        except: return ""
    ...
    def delete_account_remark(user_id, account_id):
        try:
            sg.bucketDel(bucket='gacmotor_remarks', key=f'{user_id}_{account_id}')
            return True
        except: return False
...
class AccountManager:
    ...
    def get_accounts(user_id):
        try:
            value = sg.bucketGet(bucket='gacmotor_user', key=str(user_id))
            if not value: return []
    ...
    def add_account(user_id, account):
        try:
            ...
            if account not in accounts:
                ...
                sg.bucketSet(bucket='gacmotor_user', key=str(user_id), value=str(accounts))
                return True
            return False
        except: return False
        ...
        try:
            ...
            if account in accounts:
                ...
                if accounts:
                    ...
                    sg.bucketDel(bucket='gacmotor_user', key=str(user_id))
                return True
            return False
        except: return False
    ...
    def update_account_token(account, token):
        try:
            ...
            sg.bucketSet(bucket='gacmotor_token', key=str(account), value=encrypted_token)
            return True
        except: return False
    ...
    def update_account_mobile(account, mobile):
        try:
            mobile = str(mobile or "").strip()
            if mobile:
                sg.bucketSet(bucket='gacmotor_mobile', key=str(account), value=mobile)
                return True
            return False
        except: return False
    ...
    def get_account_mobile(account):
        try:
            return sg.bucketGet(bucket='gacmotor_mobile', key=str(account)) or ""
        except: return ""
    ...
    def update_account_web_token(account, web_token):
        try:
            ...
            if web_token:
                sg.bucketSet(bucket='gacmotor_web_token', key=str(account), value=encrypt_token(web_token))
                return True
            return False
        except: return False
    ...
    def get_account_web_token(account):
        try:
            enc = sg.bucketGet(bucket='gacmotor_web_token', key=str(account))
    ...
    def get_token(account):
        try:
            enc = sg.bucketGet(bucket='gacmotor_token', key=str(account))
    ...
    def get_all_users():
        try:
            users = sg.bucketAllKeys(bucket='gacmotor_user')
            user_list = []
            for user in users:
    ...
    def migrate_account(user_id, old_account, new_account, new_token, remark=""):
        try:
            ...
            old_bind_date = sg.bucketGet(bucket='gacmotor_bind_date', key=old_account)
            if old_bind_date and not sg.bucketGet(bucket='gacmotor_bind_date', key=new_account):
                sg.bucketSet(bucket='gacmotor_bind_date', key=new_account, value=old_bind_date)
            ...
            sg.bucketSet(bucket='gacmotor_user', key=str(user_id), value=str(new_accounts))

            AccountManager.update_account_token(new_account, new_token)
            try: sg.bucketDel(bucket='gacmotor_token', key=old_account)
            except: pass
            try: sg.bucketDel(bucket='gacmotor_mobile', key=old_account)
            except: pass
            try: sg.bucketDel(bucket='gacmotor_web_token', key=old_account)
            except: pass
            try:
                pass
            except: pass
...
def process_account_binding(full_token, unique_id, nickname, remark="", aliases=None, acc_type="", legacy_key="", mobile="", web_token="", silent=False):
    try:
        ...
        if is_new:
            try: sg.bucketSet(bucket='gacmotor_bind_date', key=account, value=str(datetime.now().date()))
            except: pass
    ...
    if get_user_input() == "确认删除":
        ...
        for account in accounts:
            try:
                 ...
                 try: sg.bucketDel(bucket='gacmotor_token', key=account)
                 except: pass
                 try: sg.bucketDel(bucket='gacmotor_mobile', key=account)
                 except: pass
                 try: sg.bucketDel(bucket='gacmotor_web_token', key=account)
                 except: pass
                 try:
                     pass
                 except: pass
                 ...
                 for d in range(config['reminder_days'] + 1):
                     ...
                     try: sg.bucketDel('gacmotor_remind_log', remind_key)
                     except: pass
            except: pass
        sender.reply("✅ 批量删除完成")
            ...
            for account_item in context["accounts"]:
                ...
                if days_diff >= 0:
                    ...
                    if is_ck_valid and 0 <= days_diff <= reminder_days_cfg:
                        ...
                        has_reminded = sg.bucketGet('gacmotor_remind_log', remind_key)
...
=================="""
                            if safe_send_message(user, msg, f"到期提醒 {user}-{account}"):
                                try: sg.bucketSet('gacmotor_remind_log', remind_key, "1")
                                except: pass
                                reminded_count += 1
                    continue
...
=================="""
            ...
            if user_has_change:
                if valid_accounts:
                    try: sg.bucketSet(bucket='gacmotor_user', key=str(user), value=str(valid_accounts))
                    except: pass
                else:
                    try: sg.bucketDel(bucket='gacmotor_user', key=str(user))
                    except: pass
