...
config = plugin.Form({
    ...
    's_jx_qlname': plugin.Form.string().title('设置对接容器').default('').description('青龙容器参数用丨分割'),
    's_jx_osname': plugin.Form.string().title('青龙变量名').default('').description('青龙容器内酒仙的变量名'),
    's_jx_notify': plugin.Form.string().title('通知渠道').default('').description('检测通知推送渠道'),
    's_jx_ai_ocr_switch': plugin.Form.boolean().title('AI验证码识别').default(False).description('开启后使用AI自动识别验证码，关闭则手动输入'),
    's_jx_ai_api_url': plugin.Form.string().title('AI API地址').default('').description('默认使用硅基流动'),
    's_jx_ai_api_key': plugin.Form.string().title('AI API密钥').default('').description('硅基流动或其他兼容OpenAI API的密钥'),
    's_jx_ai_model': plugin.Form.string().title('AI模型').default('').description('视觉语言模型名称'),
})
_CONFIG_FIELD_MAP = {
