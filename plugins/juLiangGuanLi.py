...
config = plugin.Form({
    'otto_jl_imtypes': plugin.Form.string().title('jl_imtypes').default('').description('巨量签到通知平台，多个用,隔开，例如qq,wb'),
    'otto_jl_token': plugin.Form.string().title('jl_token').default('').description('请前往http://www.gxfc-s4.com使用token'),
})
_CONFIG_FIELD_MAP = {
...
def main():
    ...
    if msg == '剩余ip':
        ...
        if not token:
            sender.reply('未设置token，请前往http://www.gxfc-s4.com/r使用\n发送set otto jl_token xxxxxx')
            sys.exit()
        ...
        token = sg.get('jl_token')
        if not token:
            sg.notifyMasters('请前往http://www.gxfc-s4.com使用token\n发送set otto jl_token xxxxxx',imtypes)
            sys.exit()
