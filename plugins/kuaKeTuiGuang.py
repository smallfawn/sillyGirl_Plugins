...
config = plugin.Form({
    'quark_search_cookies': plugin.Form.string().title('夸克网盘Cookie').default('').description('夸克网盘的Cookie字符串'),
    'quark_search_save_folder': plugin.Form.string().title('保存文件夹名称').default('').description('转存时的保存文件夹名称，留空则随机生成'),
    'quark_search_share_option': plugin.Form.string().title('分享选项').default('').description('1=公开永久 2=公开1天 3=公开7天 4=公开30天 5=加密永久 6=加密1天 7=加密7天 8=加密30天'),
    'quark_search_search_api': plugin.Form.string().title('搜索API地址').default('').description('搜索API，自建项目：https://github.com/fish2018/pansou'),
    'quark_search_clean_folder_id': plugin.Form.string().title('清理文件夹ID').default('').description('需要清理的文件夹ID，留空则不启用清理功能'),
})
_CONFIG_FIELD_MAP = {
