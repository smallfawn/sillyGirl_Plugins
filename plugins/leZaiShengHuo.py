...
config = plugin.Form({
    'yuhua_lzsh_bingfa': plugin.Form.string().title('并发数量').default('').description('不填默认20'),
    'yuhua_lzsh_push_status': plugin.Form.boolean().title('推送状态').default(False).description('是否将乐仔一键运行结果推送给用户'),
    'yuhua_lzsh_debug_pwd': plugin.Form.string().title('调试模式').default('').description('非插件开发者无需理会'),
})
_CONFIG_FIELD_MAP = {
