...
const config = new plugin.Form({
  ...
  appid: plugin.Form.string().title("目标小程序 AppID").default(""),
  ...
  env_name: plugin.Form.string().title("青龙环境变量名").default("LIYUAN_TOKEN"),
});

async function main() {
  const cfg = normalize(await config.get());
  if (!cfg.enable) return s.reply("梨园插件未启用");
  try {
    ...
    return s.reply(["梨园处理完成", ...rows].join("\n"));
  } catch (error) {
    return s.reply(`梨园处理失败：${message(error)}`);
  }
}
...
async function upsertEnv(cfg, openid, value) {
  ...
  const remarks = `liYuan|${s.getPlatform()}:${s.getUserId()}|${openid}`;
...
function normalize(raw) {
  const value = raw || {};
  return { enable: value.enable !== false, smallcatId: Number(value.smallcat_id) || 1, appid: String(value.appid || "").trim(), businessUrl: String(value.business_url || "").trim(), qinglongId: Number(value.qinglong_id) || 1, envName: String(value.env_name || "LIYUAN_TOKEN").trim() || "LIYUAN_TOKEN" };
}
