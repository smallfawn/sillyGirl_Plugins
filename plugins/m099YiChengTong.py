...
config = plugin.Form({
    ...
    'mrconli_yichengtong_bind': plugin.Form.string().title('登录方式').default('').description('0：所有方式，1：仅短信登录，2：仅CK登录'),
    'mrconli_yichengtong_ql_config': plugin.Form.string().title('对接青龙').default('').description('http://ip:端口丨ClientID丨ClientSecret'),
    'mrconli_yichengtong_var_name': plugin.Form.string().title('环境变量名').default('').description('青龙容器内的变量名，默认为：m_yct'),
    'mrconli_yichengtong_is_proxy': plugin.Form.boolean().title('是否启用代理').default(False).description('true/false'),
    'mrconli_yichengtong_proxy_pool': plugin.Form.string().title('代理池地址').default('').description('代理API服务地址'),
})
_CONFIG_FIELD_MAP = {
