...
// [description: m108_潇洒桐庐凭证绑定、青龙同步、账号查询与清理]
// [depe: []]
...
const config = new plugin.Form({
  ...
  env_name: plugin.Form.string().title("脚本环境变量名").default("M108_XIAO_SA_TONG_LU"),
});

async function main() {
  try {
    const cfg = normalize(await config.get());
    if (!cfg.enable) return s.reply("m108_潇洒桐庐插件未启用");
    ...
    return s.reply("m108_潇洒桐庐：请使用登录、查询、管理或清理指令");
  } catch (error) {
    return s.reply(`m108_潇洒桐庐处理失败：${message(error)}`);
  }
}
...
async function saveAccounts(ql, envName, input, replySender) {
  try {
    ...
    return replySender.reply(`m108_潇洒桐庐同步完成：新增 ${created}，更新 ${updated}`);
  } catch (error) {
    return replySender.reply(`m108_潇洒桐庐提交失败：${message(error)}`);
  }
}
...
async function showAccounts(ql, envName) {
  ...
  if (!visible.length) return s.reply("没有找到你的m108_潇洒桐庐账号");
  return s.reply([`m108_潇洒桐庐账号：${visible.length} 个`, ...visible.map((item, index) => `${index + 1}. ${remarkOf(item) || "未备注"}${item.status ? "（已禁用）" : ""}`)].join("\n"));
}
...
async function removeAccounts(ql, envName) {
  ...
  if (!ids.length) return s.reply("没有可清理的m108_潇洒桐庐账号");
  await ql.deleteEnvs(ids);
  return s.reply(`已清理 ${ids.length} 个m108_潇洒桐庐账号`);
}
...
function ownerKey(sender) { return "m108XiaoSaTongLu|" + sender.getPlatform() + ":" + sender.getUserId(); }
...
function normalize(raw) {
  const value = raw || {};
  const envName = String(value.env_name || "M108_XIAO_SA_TONG_LU").trim();
