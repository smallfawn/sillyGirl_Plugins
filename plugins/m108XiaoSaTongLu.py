...
# [description: 无脚本提供；ck提交青龙，格式：账号#密码；仅环境变量提交青龙]
...
config = form({
    'mrconli_xstl_ql_config': form.string().title('对接青龙').default('').description('|'),
    'mrconli_xstl_var_name': form.string().title('环境变量名').default('').description('青龙容器内的变量名，默认为：xiaosa'),
    'mrconli_xstl_is_proxy': form.boolean().title('是否启用代理').default(False).description('开启代理就勾选，其实不需要代理'),
    'mrconli_xstl_proxy_pool': form.string().title('代理池地址').default('').description('代理API服务地址'),
})
_CONFIG_FIELD_MAP = {
    ('mrconli', 'xstl.ql_config'): 'mrconli_xstl_ql_config',
    ('mrconli', 'xstl.var_name'): 'mrconli_xstl_var_name',
    ('mrconli', 'xstl.is_proxy'): 'mrconli_xstl_is_proxy',
    ('mrconli', 'xstl.proxy_pool'): 'mrconli_xstl_proxy_pool',
}
...
def get_config():
    """获取插件配置"""
    try:
        var_name = sg.bucketGet(bucket_prefix, 'var_name') or "xiaosa"
        if not var_name:
