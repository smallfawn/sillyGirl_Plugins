...
config = plugin.Form({
    ...
    'mrconli_xstl_ql_config': plugin.Form.string().title('对接青龙').default('').description('|'),
    'mrconli_xstl_var_name': plugin.Form.string().title('环境变量名').default('').description('青龙容器内的变量名，默认为：xiaosa'),
    'mrconli_xstl_is_proxy': plugin.Form.boolean().title('是否启用代理').default(False).description('开启代理就勾选，其实不需要代理'),
    'mrconli_xstl_proxy_pool': plugin.Form.string().title('代理池地址').default('').description('代理API服务地址'),
})
_CONFIG_FIELD_MAP = {
