# [title: m109_小程序牛牛短剧]
# [name: m109XiaoChengXuNiuNiuDuanJu]
# [language: python]
# [class: 任务]
# [author: mrconli]
# [version: v1.0.0]
...
# [description: 无脚本提供；ck提交青龙，格式：token；仅环境变量提交青龙]
...
config = form({
    'mrconli_xnndj_ql_config': form.string().title('对接青龙').default('').description('|'),
    'mrconli_xnndj_var_name': form.string().title('环境变量名').default('').description('青龙容器内的变量名，默认为：xnndj'),
    'mrconli_xnndj_is_proxy': form.boolean().title('是否启用代理').default(False).description('开启代理就勾选，其实不需要代理'),
    'mrconli_xnndj_proxy_pool': form.string().title('代理池地址').default('').description('代理API服务地址'),
})
_CONFIG_FIELD_MAP = {
    ('mrconli', 'xnndj.ql_config'): 'mrconli_xnndj_ql_config',
    ('mrconli', 'xnndj.var_name'): 'mrconli_xnndj_var_name',
    ('mrconli', 'xnndj.is_proxy'): 'mrconli_xnndj_is_proxy',
    ('mrconli', 'xnndj.proxy_pool'): 'mrconli_xnndj_proxy_pool',
}
...
APPID = globals().get("APPID", "")
BUNDLEID = globals().get("BUNDLEID", "")
DEFAULT_UA = globals().get("DEFAULT_UA", "Mozilla/5.0")
...
def get_token_by_code(code):
    sender.reply("扫码换 token 接口未配置，请使用抓包登录")
...
def get_config():
    """获取插件配置"""
    try:
        var_name = sg.bucketGet(bucket_prefix, 'var_name') or "xnndj"
        if not var_name:
...
def main():
    ...
    if '登录' in message or '登陆' in message or '上车' in message:
        bind_choice = sg.bucketGet(bucket_prefix, 'bind') or "2"
