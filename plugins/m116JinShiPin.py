...
config = form({
    'mrconli_jinshipin_bind': form.string().title('登录方式').default('').description('0：所有方式，1：仅扫码登录，2：仅CK登录'),
    'mrconli_jinshipin_ql_config': form.string().title('对接青龙').default('').description('|'),
    'mrconli_jinshipin_var_name': form.string().title('环境变量名').default('').description('青龙容器内的变量名，默认为：m_jinshipin'),
    'mrconli_jinshipin_is_proxy': form.boolean().title('是否启用代理').default(False).description('开启代理就勾选，其实不需要代理'),
    'mrconli_jinshipin_proxy_pool': form.string().title('代理池地址').default('').description('代理API服务地址'),
})
_CONFIG_FIELD_MAP = {
