...
const config = new plugin.Form({
  ...
  task_keyword: plugin.Form.string().title("青龙任务关键词").default("美团领券PLUS"),
});

async function main() {
  try {
    const cfg = normalize(await config.get());
    if (!cfg.enable) return s.reply("美团领券PLUS插件未启用");
    ...
    return s.reply(["美团领券PLUS任务已触发", ...tasks.map((task) => `• ${task.name || task.command || task.id}`)].join("\n"));
  } catch (error) {
    return s.reply(`美团领券PLUS执行失败：${message(error)}`);
  }
}
...
function normalize(raw) {
  const value = raw || {};
  return { enable: value.enable !== false, qinglongId: Number(value.qinglong_id) || 1, taskKeyword: String(value.task_keyword || "美团领券PLUS").trim() || "美团领券PLUS" };
}
