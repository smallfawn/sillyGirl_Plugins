...
config = plugin.Form({
    ...
    'dd_nkf_run_mode': plugin.Form.string().title('运行模式(必选)').default('').description('local=本地内置任务(推荐，无需配置后台)\nsystem=提交到系统后台面板'),
    'dd_nkf_dd_nkf_qlname': plugin.Form.string().title('对接系统配置').default('').description('模式选system时必填，本地模式可留空'),
    'dd_nkf_dd_nkf_osname': plugin.Form.string().title('系统变量名').default('').description('系统容器内变量名(仅系统模式有效)'),
    'dd_nkf_enable_proxy': plugin.Form.boolean().title('是否启用代理').default(False).description('是否启用代理功能'),
    'dd_nkf_proxy_pool_url': plugin.Form.string().title('代理池地址').default('').description('代理API服务地址'),
    'dd_nkf_enable_remark': plugin.Form.boolean().title('启用备注功能').default(False).description('是否启用账号备注功能'),
})
_CONFIG_FIELD_MAP = {
