...
config = plugin.Form({
    ...
    's_nkfsj_qlname': plugin.Form.string().title('设置对接容器').default('').description('面板容器参数（兼容青龙与呆呆面板），不填则使用默认配置'),
    's_nkfsj_use_dumbpanel': plugin.Form.boolean().title('使用DumbPanel').default(False).description('勾选使用DumbPanel面板，不勾选使用青龙面板'),
    's_nkfsj_panel_group': plugin.Form.string().title('DumbPanel分组').default('').description('填写后新增/更新变量时同步写入group字段，留空则不处理'),
    's_nkfsj_osname': plugin.Form.string().title('青龙变量名').default('').description('青龙容器内的变量名'),
    's_nkfsj_notify': plugin.Form.string().title('通知渠道').default('').description('检测通知推送渠道'),
    's_nkfsj_proxy_url': plugin.Form.string().title('代理地址').default('').description('登录请求代理，支持两种模式：1)API形式(http开头的URL，GET请求返回ip:port文本) 2)代理池形式(直接填写ip:port)'),
    's_nkfsj_rsa_private_key': plugin.Form.string().title('RSA签名私钥Base64').default('').description('牛卡福接口签名私钥，留空则不执行需要签名的登录请求'),
})
_CONFIG_FIELD_MAP = {
