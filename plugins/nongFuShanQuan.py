...
config = form({
    ...
    'dd_nfsqconfig_notify': form.string().title('管理员通知').default('').description('设置接受管理员通知的渠道，如 qq,wx,tg 用英文逗号分割,不设置不推送'),
    'dd_nfsqconfig_use_amap': form.boolean().title('使用高德地图').default(False).description('开启则使用高德API解析地址(需配置key)，关闭则使用农夫山泉自带接口'),
    'dd_nfsqconfig_amap_key': form.string().title('高德地图key').default('').description('申请地址：https://console.amap.com/dev/key/app，选择Web服务'),
    'dd_nfsqconfig_default_address': form.string().title('默认地址').default('').description('设置默认运行地址，输入完整地址即可自动解析'),
    'dd_nfsqconfig_follow_lottery': form.boolean().title('农夫跟抽').default(False).description('开启后，有人中一等奖时全部账号立即使用该经纬度地址进行抽奖'),
})
_CONFIG_FIELD_MAP = {
