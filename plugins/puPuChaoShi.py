...
config = plugin.Form({
    'dd_pp_panel_type': plugin.Form.string().title('对接面板类型').default('').description('填写你当前使用的面板类型，支持：青龙、青龙面板、QL、呆呆、呆呆面板、Daidai'),
    'dd_pp_panel_config': plugin.Form.string().title('对接面板配置').default('').description('青龙：Host丨ClientID丨ClientSecret；呆呆：Host丨AppKey丨AppSecret；分隔符使用中文丨。老的 Qinglong 配置仍兼容'),
    'dd_pp_files': plugin.Form.string().title('写入脚本路径').default('').description('写入面板中的目标脚本路径，例如 leafTheFish_DeathNote/pupuCookie.txt 或 pupuCookie.txt'),
})
_CONFIG_FIELD_MAP = {
