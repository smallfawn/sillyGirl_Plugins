...
config = plugin.Form({
    'sgxw_config_sgxw_qlname': plugin.Form.string().title('设置对接容器').default('').description('你的变量需要添加到的容器？参数用丨分割'),
    'sgxw_config_sgxw_osname': plugin.Form.string().title('提交到青龙的变量名').default('').description('青龙容器内上观新闻的变量名'),
    'sgxw_config_fixed_token': plugin.Form.string().title('登录签名Token').default('').description('上观登录接口签名Token'),
})
_CONFIG_FIELD_MAP = {
