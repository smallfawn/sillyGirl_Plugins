...
config = plugin.Form({
    ...
    'mrconli_shuifeiyi_ql_config': plugin.Form.string().title('对接青龙').default('').description('|'),
    'mrconli_shuifeiyi_var_name': plugin.Form.string().title('环境变量名').default('').description('青龙容器内的变量名，默认为：m_dnys'),
    'mrconli_shuifeiyi_is_proxy': plugin.Form.boolean().title('是否启用代理').default(False).description('true/false'),
    'mrconli_shuifeiyi_proxy_pool': plugin.Form.string().title('代理池地址').default('').description('代理API服务地址'),
})
_CONFIG_FIELD_MAP = {
