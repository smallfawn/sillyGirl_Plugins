...
_CONFIG_FIELD_MAP = {
    ...
    ('G_TLG', 'ql_config'): 'G_TLG_ql_config',
    ('G_TLG', 'ql_envname'): 'G_TLG_ql_envname',
    ('G_TLG', 'daidai_config'): 'G_TLG_daidai_config',
    ('G_TLG', 'daidai_group'): 'G_TLG_daidai_group',
}
...
def handle_single_manage(account_id: str):
    ...
    f"已授权｜{expire}" if is_authorized(account_id) else ("已过期｜" + expire if expire else "未授权")
    config = get_config()
    pn = get_panel_name(config)
    ...
    for uid in users:
        ...
        if warning_list:
            ...
            try:
                ...
                pass
            try:
                sg.push('qq', '', uid, '台铃账号状态提醒', notify_msg)
                notified_users += 1
            except:
                pass
