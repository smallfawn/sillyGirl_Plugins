...
config = plugin.Form({
    ...
    's_qh_qlname': plugin.Form.string().title('设置对接容器').default('').description('面板容器参数，不填则使用默认配置'),
    's_qh_use_dumbpanel': plugin.Form.boolean().title('使用DumbPanel').default(False).description('勾选使用DumbPanel面板，不勾选使用青龙面板'),
    's_qh_osname': plugin.Form.string().title('青龙变量名').default('').description('青龙容器内茄皇的变量名'),
    's_qh_notify': plugin.Form.string().title('通知渠道').default('').description('检测通知推送渠道'),
})
_CONFIG_FIELD_MAP = {
