...
config = plugin.Form({
    'yuhua_xcbwc_share': plugin.Form.string().title('推广码子').default('').description('小蚕推广码直链'),
    'yuhua_xcbwc_bingfa': plugin.Form.string().title('并发数量').default('').description('不填默认5，若频繁提示IP已被风控请配置代理'),
    'yuhua_xcbwc_status': plugin.Form.string().title('启用代理').default('').description('0=直连，1=代理池，2=API代理'),
    'yuhua_xcbwc_proxy': plugin.Form.string().title('代理地址').default('').description('支持代理池以及API代理'),
    'yuhua_xcbwc_ip': plugin.Form.string().title('代理限制').default('').description('单IP代理次数限制，仅对API代理有效，填0为不限制'),
    'yuhua_xcbwc_push': plugin.Form.boolean().title('运行推送').default(False).description('是否将一键运行结果推送给用户'),
    'yuhua_xcbwc_hongbaoyu_push': plugin.Form.boolean().title('瓜分推送').default(False).description('是否将瓜分红包雨结果推送给用户'),
    'yuhua_xcbwc_img': plugin.Form.boolean().title('店铺头像').default(False).description('是否在店铺活动信息中显示店铺头像'),
    'yuhua_xcbwc_jiankong': plugin.Form.boolean().title('监控抢单').default(False).description('是否启用监控抢单功能'),
    'yuhua_xcbwc_debug_pwd': plugin.Form.string().title('调试模式').default('').description('非插件开发者无需理会，填入密钥开启详细日志'),
})
_CONFIG_FIELD_MAP = {
    ('yuhua_xcbwc', 'share'): 'yuhua_xcbwc_share',
    ('yuhua_xcbwc', 'bingfa'): 'yuhua_xcbwc_bingfa',
    ('yuhua_xcbwc', 'status'): 'yuhua_xcbwc_status',
    ('yuhua_xcbwc', 'proxy'): 'yuhua_xcbwc_proxy',
    ('yuhua_xcbwc', 'ip'): 'yuhua_xcbwc_ip',
    ('yuhua_xcbwc', 'push'): 'yuhua_xcbwc_push',
    ('yuhua_xcbwc', 'hongbaoyu_push'): 'yuhua_xcbwc_hongbaoyu_push',
    ('yuhua_xcbwc', 'img'): 'yuhua_xcbwc_img',
    ('yuhua_xcbwc', 'jiankong'): 'yuhua_xcbwc_jiankong',
    ('yuhua_xcbwc', 'debug_pwd'): 'yuhua_xcbwc_debug_pwd',
}
...
debug_key = sg.bucketGet('yuhua_xcbwc', 'debug_pwd') or ''
...
def get_device_data(key):
    ...
    rk=sg.bucketGet('yuhua_xcbwc_teemo',k) or k
    if d:=sg.bucketGet('yuhua_xcbwc_dev',rk):
        try:
    ...
    sg.bucketSet('yuhua_xcbwc_dev',rk,json.dumps(info))
    return info
...
def cmd_fix_risk():
    ...
    for i,acid in enumerate(accs,1):
        nm=sg.bucketGet('yuhua_xcbwc_remark',acid) or acid
        auth_status="⚠️ 未授权"
        if auth_date:='2099-12-31':
...
def sub_fix_risk(acid):
    ...
    ck=sg.bucketGet('yuhua_xcbwc_token',acid)
    if not ck: sender.reply(f"=====小蚕解限=====\n🤪 账号: {nm}\n❌ 状态: CK不存在\n==================");return
    loc_str=sg.bucketGet('yuhua_xcbwc_jiankong_weizhi',acid)
    ...
    def _execute_unlock_flow(api_obj):
        ph = sg.bucketGet('yuhua_xcbwc_phone', acid)
        if ph:
            for _ in range(3):
                try:
                    ...
                    if r1.get("status",{}).get("code")!=0 or not r1.get("pass"):
                        try:
                            sg.bucketDel('yuhua_xcbwc_phone',acid)
                        except Exception:
                            pass
                        ph=None;break
                    break
        ...
        if not ph:
            ...
            sg.bucketSet('yuhua_xcbwc_phone',acid,ph)
...
bingfa_str = sg.bucketGet('yuhua_xcbwc', 'bingfa') or ''
try:
...
def get_ip_limit():
    try: return int(sg.bucketGet('yuhua_xcbwc', 'ip') or '0')
    except: return 0
def is_monitor_enabled(): return (sg.bucketGet('yuhua_xcbwc', 'jiankong') or 'false').lower() == 'true'
...
def get_accounts(uid):
    data = sg.bucketGet('yuhua_xcbwc_user', uid)
    if not data: return []
    try: return json.loads(data)
    except: return []
def set_accounts(uid, arr): sg.bucketSet('yuhua_xcbwc_user', uid, json.dumps(arr))
...
class BaseApiClient:
    def __init__(self, cookie_str):
        ...
        self.lng = None
        try:
            loc_json = sg.bucketGet('yuhua_xcbwc_jiankong_weizhi', self.teemo)
            if loc_json:
...
def format_promotion_list(promotions, reduce_left_num=False):
    ...
    show_img = (sg.bucketGet('yuhua_xcbwc', 'img') or 'false') == 'true'
    grouped = {}
...
def monitor_task():
    ...
    for u in sg.bucketAllKeys('yuhua_xcbwc_user'):
        for acid in get_accounts(u):
            if (sg.bucketGet('yuhua_xcbwc_jiankong_status', acid) or 'off') != 'on': continue
...
def process_location_group(group):
    ...
    for p in promos:
        pf = determine_platform(p)
        if pf in STORE_BUCKET: sg.bucketSet(STORE_BUCKET[pf], str(p["store"]["store_id"]), json.dumps({"store_name": p["store"].get("name", "")}, ensure_ascii=False))
    for u, acid, ck in group["accs"]:
        try:
            mon_stores = {pf: set(json.loads(sg.bucketGet(b, acid) or "[]")) for pf, b in MON_BUCKET.items()}
            def _get_today_data(bucket_name):
                default_struct = {"date": current_date, "store_ids": []}
                try:
                    raw = sg.bucketGet(bucket_name, acid)
            ...
            push_st = sg.bucketGet('yuhua_xcbwc_jiankong_push', acid) or 'f'
            nm = sg.bucketGet('yuhua_xcbwc_remark', acid) or acid
            ...
            if new_ids:
                ...
                sg.bucketSet('yuhua_xcbwc_jiankong_todayStoreId', acid, json.dumps(notified_data, ensure_ascii=False))
                if push_st == 't':
            ...
            for (sid, pf), plist in store_map.items():
                ...
                if succ:
                    ...
                    sg.bucketSet('yuhua_xcbwc_grabbed_store_today', acid, json.dumps(gj, ensure_ascii=False))
                    push_msg(u, f"=====抢单成功=====\n🤪 账号: {nm}\n🎯 店铺: {sid}\n------------------\n{format_promotion_list([bestp], reduce_left_num=True)}\n==================")
                else:
                    if "超过店铺限制" in info:
                        overlimit.add(sid); ol["store_ids"] = list(overlimit); ol["date"] = current_date
                        sg.bucketSet('yuhua_xcbwc_overlimit_store_today', acid, json.dumps(ol, ensure_ascii=False))
                    if "无法重复参与" in info:
                        repeat.add(sid); rp["store_ids"] = list(repeat); rp["date"] = current_date
                        sg.bucketSet('yuhua_xcbwc_repeat_store_today', acid, json.dumps(rp, ensure_ascii=False))
...
def do_execute_all():
    ...
    clear_temp_ip_records()
    try:
        push_enabled = (sg.bucketGet('yuhua_xcbwc', 'push') or 'false') == 'true'
        sender.reply("正在运行...")
        tasks = [(u, acid) for u in sg.bucketAllKeys('yuhua_xcbwc_user') for acid in get_accounts(u)]
        total = len(tasks)
        def worker(u, acid):
            nm = sg.bucketGet('yuhua_xcbwc_remark', acid) or acid
            try:
...
zsm = sg.bucketGet('yuhua_xcbwc','zsm') or ''
price_str = sg.bucketGet('yuhua_xcbwc','price') or '0'
...
def sub_run(acid):
    execute_with_account(acid, "运行", lambda obj, acid: format_result(sg.bucketGet('yuhua_xcbwc_remark', acid) or acid, "运行", f"\n{format_success_logs(obj.run_all())}", True))
    ...
    try:
        ...
        while True:
            ...
            for pm in pros:
                ...
                if pf in {1, 2, 3}: sg.bucketSet({1: 'yuhua_xcbwc_store_mt_info', 2: 'yuhua_xcbwc_store_elm_info', 3: 'yuhua_xcbwc_store_jd_info'}[pf], str(pm["store"]["store_id"]), json.dumps({"store_name": pm["store"].get("name","")}, ensure_ascii=False))
...
def cmd_order_list_all():
    ...
    while True:
        ...
        cli = YuhuaClient(sg.bucketGet('yuhua_xcbwc_token', acid))
...
def sub_activity_monitor(acid):
    ...
    if not sg.bucketGet('yuhua_xcbwc_jiankong_weizhi', acid): sender.reply(f"=====监控抢单=====\n🤪 账号: {nm}\n💫 结果: 未记录位置\n=================="); return
    MON_BUCKET = {1: 'yuhua_xcbwc_jiankong_store_mt', 2: 'yuhua_xcbwc_jiankong_store_elm', 3: 'yuhua_xcbwc_jiankong_store_jd'}
    SYM = {1: "ⓜ", 2: "ⓔ", 3: "ⓙ"}
    def _load_list(bname):
        try: return json.loads(sg.bucketGet(bname, acid) or "[]")
        except: return []
    while True:
        st, push_st = sg.bucketGet('yuhua_xcbwc_jiankong_status', acid) or 'off', sg.bucketGet('yuhua_xcbwc_jiankong_push', acid) or 'f'
        store_seq = [(sid, pf) for pf, b in MON_BUCKET.items() for sid in _load_list(b)]
        def _today(name):
            try: d = json.loads(sg.bucketGet(name, acid) or '{"store_ids":[]}'); return set(d["store_ids"]) if d.get("date") == str(get_china_date()) else set()
            except: return set()
        ...
        if store_seq:
            for idx, (sid, pf) in enumerate(store_seq, 1):
                store_info = sg.bucketGet({1:'yuhua_xcbwc_store_mt_info', 2:'yuhua_xcbwc_store_elm_info', 3:'yuhua_xcbwc_store_jd_info'}[pf], sid) or "{}"
        ...
        if not cmd: return
        cmd = cmd.lower()
        if cmd in ('on', 'off'): sg.bucketSet('yuhua_xcbwc_jiankong_status', acid, cmd); sender.reply(f"✅ 已{'开启' if cmd=='on' else '关闭'}监控")
        elif cmd in ('t', 'f'):
            if st != 'on': sender.reply("❌ 请先开启监控抢单")
            else: sg.bucketSet('yuhua_xcbwc_jiankong_push', acid, cmd); sender.reply("✅ 已修改上新推送状态")
        elif cmd.startswith('+'):
            sid = cmd[1:].strip()
            ...
            lst.append(sid); sg.bucketSet(MON_BUCKET[pf_val], acid, json.dumps(lst)); sender.reply(f"✅ 店铺[{sid}]添加成功")
        elif cmd.startswith('-'):
            vs = cmd[1:].strip()
            ...
            if sid in lst: lst.remove(sid); sg.bucketSet(MON_BUCKET[pf], acid, json.dumps(lst))
    ...
    try:
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                sg.bucketSet('yuhua_xcbwc_jiankong_weizhi', acid, json.dumps(loc_data))
                sender.reply(f"=====记录位置=====\n🤪 账号: {nm}\n✅ 状态: 记录成功\n🎨 经度: {la}\n🃏 纬度: {ln}\n🏙️ 城市编码: {cc}\n==================")
                return True
            except Exception as e:
                remaining = max_retries - 1 - attempt
...
def cmd_login():
    ...
    if choice == "2":
        ...
        if share := sg.bucketGet('yuhua_xcbwc', 'share') or '': sender.replyImage(share)
        inp = get_user_input(None)
        if not inp: return
        parts = inp.split("#")
...
def _store_login_info(remark, vayne, teemo, sivir, uid):
    ...
    existed_aid = next((aid for aid in get_accounts(uid) if (ck := sg.bucketGet('yuhua_xcbwc_token', aid)) and ck.split('#')[1] == teemo), None)
    if not existed_aid: existed_aid = next((aid for aid in get_accounts(uid) if sg.bucketGet('yuhua_xcbwc_teemo', aid) == teemo), None)
    target_acid = existed_aid
    is_new_account = False
    if target_acid:
        sg.bucketSet('yuhua_xcbwc_token', target_acid, ck_str)
        sg.bucketSet('yuhua_xcbwc_teemo', target_acid, teemo)
        sg.bucketSet('yuhua_xcbwc_fail_count', target_acid, '0')
        if not (existing_remark := sg.bucketGet('yuhua_xcbwc_remark', target_acid)) or existing_remark == target_acid:
            sg.bucketSet('yuhua_xcbwc_remark', target_acid, remark)
    else:
        is_new_account = True
        ...
        sg.bucketSet('yuhua_xcbwc_remark', target_acid, remark)
        sg.bucketSet('yuhua_xcbwc_fail_count', target_acid, '0')
    loc_record = sg.bucketGet('yuhua_xcbwc_jiankong_weizhi', target_acid)
    final_remark = sg.bucketGet('yuhua_xcbwc_remark', target_acid) or remark
...
def cmd_manage():
    ...
    lines = ["=====账号列表=====", "[0] 授权全部账号", "------------------"]
    for i, acid in enumerate(accs, 1):
        nm = sg.bucketGet('yuhua_xcbwc_remark', acid) or f"账号{i}"
        auth_status = "⚠️ 未授权"
        if auth_date := '2099-12-31':
            try: auth_status = f"✅ {auth_date}" if datetime.strptime(auth_date, "%Y-%m-%d").date() >= today else "❌ 已过期"
            except: pass
...
def auth_user():
    ...
    if idx == 0: sender.reply(f"=====授权完成=====\n✅ 成功: {success_count}个账号\n❌ 失败: {fail_count}个账号\n⏰ 时长: {'授权' if days > 0 else '扣除'}{abs(days)}天\n==================")
        ...
        rm = sg.bucketGet('yuhua_xcbwc_remark', acid) or acid
        new_date = '2099-12-31'
        sender.reply(f"=====授权成功=====\n🤪 账号: {rm}\n⏰ 时长: {'授权' if days > 0 else '扣除'}{abs(days)}天\n📅 到期: {new_date}\n==================")
...
def _get_hb_events():
    ...
    for u in sg.bucketAllKeys('yuhua_xcbwc_user'):
        for acid in get_accounts(u):
            if ck := sg.bucketGet('yuhua_xcbwc_token', acid):
                city_code = 440304
                if loc := sg.bucketGet('yuhua_xcbwc_jiankong_weizhi', acid):
...
def get_current_hb_event():
    ...
    if data.get("date") != today_str:
        ...
        sg.bucketSet('yuhua_xcbwc_hb_eventid', 'events', json.dumps(data, ensure_ascii=False))
...
def run_single_account_for_rain(user_id, acid, start_ts):
    ...
    try:
        ...
        if "CK失效" in str(e):
            try:
                sg.bucketDel('yuhua_xcbwc_token', acid)
            except Exception:
                pass
            try:
                sg.bucketDel('yuhua_xcbwc_fail_count', acid)
            except Exception:
                pass
    ...
    city_code = 440304
    if loc_str := sg.bucketGet('yuhua_xcbwc_jiankong_weizhi', acid):
        try: city_code = int(json.loads(loc_str).get("city_code", 440304))
        except: pass
...
def do_redPackRainAll():
    if not sender.isAdmin(): sender.reply("❌ 需要管理员权限"); return
    try:
        global_push_enabled = (sg.bucketGet('yuhua_xcbwc', 'hongbaoyu_push') or 'false') == 'true'
        sender.reply("正在运行...")
            ...
            with ThreadPoolExecutor(max_workers=MAX_WORKERS) as ex:
                ...
                for future in as_completed(future_map):
                    try: results_with_task_info.append((future_map[future], future.result()))
                        ...
                        nm_err = sg.bucketGet('yuhua_xcbwc_remark', acid_err) or acid_err
        ...
        for task_info, result_data in results_with_task_info:
            ...
            if not status and details.get("need_close_after_push", False):
                try:
                    sg.bucketSet('yuhua_xcbwc_hongbaoyu_status', acid, 'off')
                    if global_push_enabled and (sg.bucketGet(push_bucket, acid) or "off") == 'on':
                        push_msg(user_id, f"=====瓜分结果=====\n🤪 账号: {name}\n🪁 原因: {details['final_reason']}\n==================")
                        sg.bucketSet(push_bucket, acid, 'off')
                except Exception: pass
            elif global_push_enabled and (sg.bucketGet(push_bucket, acid) or "off") == 'on' and status != "skip":
                push_msg(user_id, f"=====瓜分结果=====\n🤪 账号: {name}\n{'💫 结果' if status else '🪁 原因'}: {details['final_reason']}\n==================")

        with stats_update_lock:
            for acid, updates in stats_updates.items():
                stats_data_str = sg.bucketGet(stats_bucket, acid)
                ...
                sg.bucketSet(stats_bucket, acid, json.dumps(stats))
...
def handle_ck_result(acid, is_success):
    if is_success:
        sg.bucketSet('yuhua_xcbwc_fail_count', acid, '0')
def cmd_manage_hongbaoyu_for_account(acid):
    ...
    def display_menu_hongbaoyu():
        feature_status = sg.bucketGet(status_bucket, key) or "on"
        push_status = sg.bucketGet(push_bucket, key) or "off"
        stats_data_str = sg.bucketGet(stats_bucket, key)
    ...
    while True:
        ...
        if user_input in ('on', 'off'):
            if feature_status != user_input:
                sg.bucketSet(status_bucket, key, user_input)
            ...
            if push_status != new_push_status:
                sg.bucketSet(push_bucket, key, new_push_status)
