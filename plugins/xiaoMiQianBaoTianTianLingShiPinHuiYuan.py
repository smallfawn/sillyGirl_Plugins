# [title: 小米钱包-天天领视频会员]
# [name: xiaoMiQianBaoTianTianLingShiPinHuiYuan]
# [language: python]
# [class: 任务]
# [author: yuhualhh]
# [version: v3.3.3]
...
config = form({
    'yuhua_xmqb_bingfa': form.string().title('任务并发').default('').description('不填默认20'),
    'yuhua_xmqb_qiangdui_bingfa': form.string().title('抢兑并发').default('').description('不填默认20'),
    'yuhua_xmqb_status': form.boolean().title('推送状态').default(False).description('是否将米包一键运行结果推送给用户'),
    'yuhua_xmqb_enable_download': form.boolean().title('拉新任务').default(False).description('是否启用视频专区拉新任务(下载试用)，默认关闭以降低风险'),
    'yuhua_xmqb_enable_2day_task': form.boolean().title('2d拉新任务').default(False).description('是否启用视频专区2天拉新任务(下载试用2天)，默认关闭以降低风险'),
})
_CONFIG_FIELD_MAP = {
    ('yuhua_xmqb', 'bingfa'): 'yuhua_xmqb_bingfa',
    ('yuhua_xmqb', 'qiangdui_bingfa'): 'yuhua_xmqb_qiangdui_bingfa',
    ('yuhua_xmqb', 'status'): 'yuhua_xmqb_status',
    ('yuhua_xmqb', 'enable_download'): 'yuhua_xmqb_enable_download',
    ('yuhua_xmqb', 'enable_2day_task'): 'yuhua_xmqb_enable_2day_task',
}
...
def _get_or_create_device_params(acc_unique_id):
    ...
    stored_data = sg.bucketGet(bucket_key, acc_unique_id)
    if stored_data:
        try:
    ...
    if encrypted_params:
        sg.bucketSet(bucket_key, acc_unique_id, encrypted_params)
    return params
_offset_expiry = 0
...
def _get_display_name(acc_unique_id: str) -> str:
    user_remark = sg.bucketGet(f'{bucket_prefix}_remark', acc_unique_id)
    if user_remark and user_remark.strip():
        return user_remark.strip()
    original_phone = sg.bucketGet(f'{bucket_prefix}_phone_id', acc_unique_id) or "未知号码"
...
回复"q"退出"""
    ...
    if not new_remark_str:
        sender.reply("❌ 备注不能为空")
        return
    sg.bucketSet(f'{bucket_prefix}_remark', acc_unique_id, new_remark_str)
...
def qr_login_mibao(sender, userid, user_config):
    ...
    try:
        ...
        try:
            ...
            shortener_service_url = f"http://s.sillygirl.cn/s?url={encoded_original_url}"
        ...
        try:
            ...
            if new_service_token_val:
                ...
                if self.acc_unique_id:
                    ...
                    True
                    sg.bucketSet(f'{bucket_prefix}_token2', self.acc_unique_id, storage_biz_ck)
            ...
            if res:
                ...
                if error_msg:
                    if '您的账号存在安全风险' in error_msg:
                        sg.bucketSet(f'{bucket_prefix}_risk_accounts', self.acc_unique_id, 'true')
...
def get_config():
    config = {
        ...
        'price_str': sg.bucketGet(bucket_prefix, 'price') or '0',
    ...
    config['task_bingfa_str'] = sg.bucketGet(bucket_prefix, 'bingfa') or '20'
    try:
        ...
        config['task_bingfa'] = 20
    config['qiangdui_bingfa_str'] = sg.bucketGet(bucket_prefix, 'qiangdui_bingfa') or '20'
    try:
        ...
        config['qiangdui_bingfa'] = 20
    config['push_status'] = sg.bucketGet(bucket_prefix, 'status') == 'true'
    config['enable_download_task'] = sg.bucketGet(bucket_prefix, 'enable_download') == 'true'
    config['enable_2day_task'] = sg.bucketGet(bucket_prefix, 'enable_2day_task') == 'true'
    return config
...
def query_mibaob(sender, userid, user_config):
    user_accounts_list = _sg_literal(sg.bucketGet(f'{bucket_prefix}_user_accounts', userid) or '[]')
...
=================="""); return
    ...
    for acc_unique_id in user_accounts_list:
        ...
        service_token_ck = sg.bucketGet(f'{bucket_prefix}_token2', acc_unique_id)
        pass_token_phone_ck = sg.bucketGet(f'{bucket_prefix}_token', acc_unique_id)
...
def _cookie_login_inner(sender, userid, user_config, ck_raw, phone_override: str | None = None):
    ...
    accounts_list_str = sg.bucketGet(f"{bucket_prefix}_user_accounts", userid) or "[]"
    try:
        user_accounts = _sg_literal(accounts_list_str)
        if not isinstance(user_accounts, list): user_accounts = []
    except:
        user_accounts = []
    target_account_unique_id = None
    for acc_uid_candidate in user_accounts:
        if sg.bucketGet(f"{bucket_prefix}_user_mapping", acc_uid_candidate) == true_user_id:
    ...
    if is_new_account_mapping:
        ...
        sg.bucketSet(f"{bucket_prefix}_user_accounts", userid, str(user_accounts))
        sg.bucketSet(f"{bucket_prefix}_user_mapping", target_account_unique_id, true_user_id)
    sg.bucketSet(f"{bucket_prefix}_token", target_account_unique_id, ck_norm)
    sg.bucketSet(f"{bucket_prefix}_token2", target_account_unique_id, service_ck_complete)
    sg.bucketSet(f"{bucket_prefix}_phone_id", target_account_unique_id, actual_phone_identifier)
    user_remark = sg.bucketGet(f'{bucket_prefix}_remark', target_account_unique_id)
...
回复"q"取消""")
        ...
        if not new_remark_input:
            ...
            sg.bucketSet(f'{bucket_prefix}_remark', target_account_unique_id, str(new_remark_input).strip())
        else:
            sender.reply("✅ 跳过设置备注")
...
def delete_single_account(sender, userid, user_config, acc_unique_id):
    ...
    if acc_unique_id in user_accounts_list:
        ...
        if not user_accounts_list:
            try:
                sg.bucketDel(f'{bucket_prefix}_user_accounts', userid)
            except Exception:
                pass
        else:
            sg.bucketSet(f'{bucket_prefix}_user_accounts', userid, str(user_accounts_list))
    try:
        sg.bucketDel(f'{bucket_prefix}_token', acc_unique_id)
    except Exception:
        pass
    try:
        sg.bucketDel(f'{bucket_prefix}_token2', acc_unique_id)
    except Exception:
        pass
    try:
        pass
        ...
        pass
    try:
        sg.bucketDel(f'{bucket_prefix}_phone_id', acc_unique_id)
    except Exception:
        pass
    try:
        sg.bucketDel(f'{bucket_prefix}_user_mapping', acc_unique_id)
    except Exception:
        pass
    try:
        sg.bucketDel(f'{bucket_prefix}_remark', acc_unique_id)
    except Exception:
        pass
    try:
        sg.bucketDel(f'{bucket_prefix}_device_ids', acc_unique_id)
    except Exception:
        pass
...
def admin_run_all_xiaomi_tasks(sender, user_config):
    ...
    all_sduck_users = sg.bucketAllKeys(f'{bucket_prefix}_user_accounts')
    for sduck_userid_map in all_sduck_users:
        sduck_user_acc_list_str = sg.bucketGet(f'{bucket_prefix}_user_accounts', sduck_userid_map)
        if sduck_user_acc_list_str:
            try:
                sduck_user_acc_list = _sg_literal(sduck_user_acc_list_str)
                for acc_id_map in sduck_user_acc_list:
                    original_phone_db = sg.bucketGet(f'{bucket_prefix}_phone_id', acc_id_map) or f"未知_{acc_id_map}"
                    pass_token_ck_db = sg.bucketGet(f'{bucket_prefix}_token', acc_id_map)
                    service_token_ck_db = sg.bucketGet(f'{bucket_prefix}_token2', acc_id_map)
                    if pass_token_ck_db:
...
def admin_detect_all_accounts(sender, user_config):
    ...
    for sduck_userid_detect in all_sduck_users:
        user_accounts_list_detect = _sg_literal(sg.bucketGet(f'{bucket_prefix}_user_accounts', sduck_userid_detect) or '[]')
        for acc_unique_id_detect in user_accounts_list_detect:
            ...
            original_phone_detect = sg.bucketGet(f'{bucket_prefix}_phone_id', acc_unique_id_detect) or "未知号码"
            service_token_ck = sg.bucketGet(f'{bucket_prefix}_token2', acc_unique_id_detect)
            pass_token_phone_ck = sg.bucketGet(f'{bucket_prefix}_token', acc_unique_id_detect)
...
def clean_expired_or_invalid_mibaob(sender, user_config):
    ...
    all_sduck_users = sg.bucketAllKeys(f'{bucket_prefix}_user_accounts') or []
    for sduck_userid in all_sduck_users:
        user_accounts_list_str = sg.bucketGet(f'{bucket_prefix}_user_accounts', sduck_userid)
        current_user_accounts_orig = []
        try:
            parsed_list = _sg_literal(user_accounts_list_str or '[]')
        ...
        for acc_unique_id in current_user_accounts_orig:
            ...
            if sg.bucketGet(f'{bucket_prefix}_risk_accounts', acc_unique_id):
                display_name = _get_display_name(acc_unique_id)
                auth_expiry = '2099-12-31' or "未知"
                push_message = f"""=====米包清理=====
🤪 用户账号: {display_name}
☁️ 授权到期: {auth_expiry}
🪁 清理原因: 您的账号存在安全风险
=================="""
            ...
            auth_time_str_clean = '2099-12-31'
        ...
        if len(valid_accounts_for_this_user_new) != len(current_user_accounts_orig):
            if not valid_accounts_for_this_user_new:
                try:
                    sg.bucketDel(f'{bucket_prefix}_user_accounts', sduck_userid)
                except Exception:
                    pass
            else:
                sg.bucketSet(f'{bucket_prefix}_user_accounts', sduck_userid, str(valid_accounts_for_this_user_new))
    ...
    for bucket_name in data_bucket_names:
        keys_from_this_bucket = sg.bucketAllKeys(bucket_name) or []
    ...
    if accounts_to_delete_final:
        for acc_id_to_delete in accounts_to_delete_final:
            try:
                sg.bucketDel(f'{bucket_prefix}_token', acc_id_to_delete)
            except Exception:
                pass
            try:
                sg.bucketDel(f'{bucket_prefix}_token2', acc_id_to_delete)
            except Exception:
                pass
            try:
                pass
                ...
                pass
            try:
                sg.bucketDel(f'{bucket_prefix}_phone_id', acc_id_to_delete)
            except Exception:
                pass
            try:
                sg.bucketDel(f'{bucket_prefix}_user_mapping', acc_id_to_delete)
            except Exception:
                pass
            try:
                sg.bucketDel(f'{bucket_prefix}_remark', acc_id_to_delete)
            except Exception:
                pass
            try:
                sg.bucketDel(f'{bucket_prefix}_risk_accounts', acc_id_to_delete)
            except Exception:
                pass
            cleaned_count += 1
...
回复"q"退出""")
    ...
    all_users = sg.bucketAllKeys(f'{bucket_prefix}_user_accounts')
    if not all_users:
        sender.reply(f"""
=====操作完成=====
✅ 成功: 0个账号
...
==================""")
    ...
    for current_userid in all_users:
        user_accounts = _sg_literal(sg.bucketGet(f'{bucket_prefix}_user_accounts', current_userid) or '[]')
        for acc_unique_id in user_accounts:
            processed_accounts += 1
            time.sleep(0.05)
            current_auth_str_loop = '2099-12-31'
...
def push_message_to_user(user_id, message):
    ...
    try:
        ...
        for platform in platforms:
            sg.push(platform, '', user_id, '', message)
            time.sleep(0.1)
    except Exception as e:
        pass
...
def show_exchange_products(sender, userid, user_config, acc_unique_id):
    display_name = _get_display_name(acc_unique_id)
    service_token_ck = sg.bucketGet(f'{bucket_prefix}_token2', acc_unique_id)
    pass_token_ck = sg.bucketGet(f'{bucket_prefix}_token', acc_unique_id)
...
==================""")
        return
    original_phone = sg.bucketGet(f'{bucket_prefix}_phone_id', acc_unique_id) or "未知"
...
==================""")
        return
    updated_service_token_ck = sg.bucketGet(f'{bucket_prefix}_token2', acc_unique_id)
...
==================""")
    ...
    current_exchange_prize = sg.bucketGet(f'{bucket_prefix}_exchange', acc_unique_id)
...
=================="""
    ...
    if choice_str.lower() == 'd':
        try:
            sg.bucketDel(f'{bucket_prefix}_exchange', acc_unique_id)
        except Exception:
            pass
        try:
            sg.bucketDel(f'{bucket_prefix}_exchange_phone', acc_unique_id)
        except Exception:
            pass
...
==================""")
        return
    existing_exchange = sg.bucketGet(f'{bucket_prefix}_exchange', acc_unique_id)
    sg.bucketSet(f'{bucket_prefix}_exchange', acc_unique_id, prize_name)
    sg.bucketSet(f'{bucket_prefix}_exchange_phone', acc_unique_id, phone1)
...
def handle_xiaomi_yijian_qiangdui():
    ...
    all_exchange_keys = sg.bucketAllKeys(f'{bucket_prefix}_exchange')
    ...
    all_users = sg.bucketAllKeys(f'{bucket_prefix}_user_accounts')
    for u in all_users:
        acc_list = _sg_literal(sg.bucketGet(f'{bucket_prefix}_user_accounts', u) or '[]')
        for ac in acc_list:
            owner_map[ac] = u
    concurrency_data = []
    fail_reasons = []
    ...
    for acc_id in all_exchange_keys:
        if STOP_EXCHANGE:
            ...
            return
        prize_name = sg.bucketGet(f'{bucket_prefix}_exchange', acc_id)
        target_phone = sg.bucketGet(f'{bucket_prefix}_exchange_phone', acc_id)
        if not prize_name or not target_phone:
            try:
                sg.bucketDel(f'{bucket_prefix}_exchange', acc_id)
            except Exception:
                pass
            try:
                sg.bucketDel(f'{bucket_prefix}_exchange_phone', acc_id)
            except Exception:
                pass
        ...
        if not check_account_authorization(acc_id):
            if display_name == "未知号码":
                ...
                continue
        service_token_ck = sg.bucketGet(f'{bucket_prefix}_token2', acc_id)
        pass_token_ck = sg.bucketGet(f'{bucket_prefix}_token', acc_id)
        if not service_token_ck and not pass_token_ck:
            fail_reasons.append(f"【{display_name}】无CK或已失效")
            continue
        original_phone = sg.bucketGet(f'{bucket_prefix}_phone_id', acc_id) or "未知"
        ...
        if not ck_valid:
            ...
            continue
        updated_service_token_ck = sg.bucketGet(f'{bucket_prefix}_token2', acc_id)
...
def _get_dynamic_s_token(sender, force_refresh=False, banned_token=None):
    ...
    f'{bucket_prefix}_s_token_cache'

    if not force_refresh:
        cached_token = sg.bucketGet(bucket_prefix, 's_token_cache')
        if cached_token:
    ...
    sg.bucketSet(bucket_prefix, 's_token_cache', fetched_token)
...
if __name__ == "__main__":
    try:
        ...
        try:
            temp_sender = sg.Sender(sg.getSenderID())
            if temp_sender:
