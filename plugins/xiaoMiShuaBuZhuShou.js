...
// [description: 小米刷步助手凭证绑定、青龙同步、账号查询与清理]
// [depe: []]
...
const config = new plugin.Form({
  ...
  env_name: plugin.Form.string().title("脚本环境变量名").default("XIAOMI_STEPS"),
});

async function main() {
  try {
    const cfg = normalize(await config.get());
    if (!cfg.enable) return s.reply("小米刷步助手插件未启用");
    ...
    if (/清理|删除|解绑/.test(content)) return removeAccounts(ql, cfg.envName);
    if (/^刷步$|一键刷步/.test(content)) return runTasks(ql);
    ...
    return s.reply("小米刷步助手：请使用登录、查询、管理或清理指令");
  } catch (error) {
    return s.reply(`小米刷步助手处理失败：${message(error)}`);
  }
}
...
async function saveAccounts(ql, envName, input, replySender) {
  try {
    ...
    return replySender.reply(`小米刷步助手同步完成：新增 ${created}，更新 ${updated}`);
  } catch (error) {
    return replySender.reply(`小米刷步助手提交失败：${message(error)}`);
  }
}
...
async function showAccounts(ql, envName) {
  ...
  if (!visible.length) return s.reply("没有找到你的小米刷步助手账号");
  return s.reply([`小米刷步助手账号：${visible.length} 个`, ...visible.map((item, index) => `${index + 1}. ${remarkOf(item) || "未备注"}${item.status ? "（已禁用）" : ""}`)].join("\n"));
}
...
async function removeAccounts(ql, envName) {
  ...
  if (!ids.length) return s.reply("没有可清理的小米刷步助手账号");
  await ql.deleteEnvs(ids);
  return s.reply(`已清理 ${ids.length} 个小米刷步助手账号`);
}

async function runTasks(ql) {
  const response = await ql.request("GET", "/crons", null, { searchValue: "小米刷步" });
  let tasks = response;
  for (let i = 0; i < 4 && tasks && !Array.isArray(tasks); i += 1) tasks = tasks.data ?? tasks.items ?? tasks.list;
  const ids = (Array.isArray(tasks) ? tasks : []).map((item) => item.id ?? item._id).filter((id) => id !== undefined && id !== null);
  if (!ids.length) return s.reply("未找到青龙小米刷步任务");
  await ql.request("PUT", "/crons/run", ids);
  return s.reply(`已触发 ${ids.length} 个小米刷步任务`);
}
...
function ownerKey(sender) { return "xiaoMiShuaBuZhuShou|" + sender.getPlatform() + ":" + sender.getUserId(); }
...
function normalize(raw) {
  const value = raw || {};
  const envName = String(value.env_name || "XIAOMI_STEPS").trim();
