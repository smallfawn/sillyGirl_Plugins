...
config = form({
    'Joh_Shuabu_config_choice': form.string().title('代理选择').default('').description('填1代理池，填2API代理。不填不走代理。注：不用代理多人使用容易登录失败或刷步失败'),
    'Joh_Shuabu_config_proxy': form.string().title('代理池').default('').description('本地代理池链接'),
    'Joh_Shuabu_config_proxy_api': form.string().title('API代理').default('').description('API代理链接'),
})
_CONFIG_FIELD_MAP = {
