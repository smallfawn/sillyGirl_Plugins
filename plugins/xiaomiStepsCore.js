// [title: 小米步数公共模块]
// [name: xiaomiStepsCore]
// [desc: 小米运动登录、Token 与步数数据构造公共能力]
// [author: sillyGirl]
// [version: v1.0.0]
// [status: true]
// [admin: false]
...
"use strict";
const USER_AGENT = "MiFit/6.12.0 (Android; 16; Density/1.5)";
...
const DEVICE_ID = "fuck1069-2002-7869-0129-757geoi6sam1";
const BAND_ID = "DA932FFFFE8816E7";
const DATA_TEMPLATE =
...
async function request(url, options = {}) {
  ...
  const timer = setTimeout(() => controller.abort(), Number(options.timeout) || 15000);
  try {
...
async function login(user, password, options = {}) {
  const account = String(user || "").trim(),
    passwd = String(password || ""),
    email = account.includes("@"),
    apiUser = email ? account : `+86${account}`;
  const headers = {
    "content-type": "application/x-www-form-urlencoded; charset=UTF-8",
    "user-agent": USER_AGENT,
    app_name: APP_NAME,
  };
  const body1 = `client_id=HuaMi&country_code=CN&json_response=true&name=${apiUser}&password=${passwd}&redirect_uri=https://s3-us-west-2.amazonaws.com/hm-registration/successsignin.html&state=REDIRECTION&token=access`;
  const first = await request(`https://api-user.zepp.com/registrations/${encodeURIComponent(apiUser)}/tokens`, {
    ...options,
    method: "POST",
    headers,
    body: body1,
  ...
  if (first.status === 429) return { loginToken: null, userId: null, message: "登录过于频繁，请更换IP或稍后再试" };
  const code = first.data?.access;
  if (!code)
    return {
      loginToken: null,
      userId: null,
      message: first.data?.message || first.data?.error || `第一阶段登录失败（HTTP ${first.status}）`,
    };
  const body2 = `app_name=${APP_NAME}&country_code=CN&code=${code}&device_id=${DEVICE_ID}&device_model=android_phone&app_version=6.12.0&grant_type=access_token&allow_registration=false&dn=${DOMAINS}&source=${APP_NAME}&third_name=${email ? "huami" : "huami_phone"}`;
  const second = await request("https://account.zepp.com/v2/client/login", {
    ...options,
    method: "POST",
    headers,
    body: body2,
  });
  const token = second.data?.token_info || {};
  return {
    loginToken: token.login_token || null,
    userId: token.user_id || null,
    message: second.data?.message || second.data?.error || `第二阶段登录失败（HTTP ${second.status}）`,
  };
}

async function getAppToken(loginToken, options = {}) {
  const url = new URL("https://account.zepp.com/v1/client/app_tokens");
  url.searchParams.set("app_name", APP_NAME);
  url.searchParams.set("dn", DOMAINS);
  url.searchParams.set("login_token", loginToken);
  const response = await request(url, { ...options, headers: { "user-agent": USER_AGENT } });
  return {
    appToken: response.data?.token_info?.app_token || null,
    message: response.data?.message || response.data?.error || `获取 app_token 失败（HTTP ${response.status}）`,
  };
}

function renderData(step, date = new Date()) {
  const day = [
    date.getFullYear(),
    String(date.getMonth() + 1).padStart(2, "0"),
    String(date.getDate()).padStart(2, "0"),
  ].join("-");
  return DATA_TEMPLATE.replace(/(?<=date%22%3A%22)[^%]*(?=%22%2C%22data)/, day).replace(
    /(?<=ttl%5C%22%3A)\d+(?=%2C%5C%22dis)/,
    String(step),
  );
}

async function changeSteps(userId, appToken, step, options = {}) {
  const value = Number(step);
  if (!Number.isInteger(value) || value < 1 || value > 98800) return { ok: false, message: "步数范围必须是 1-98800" };
  const dataJson = renderData(value, options.date || new Date());
  const body = `userid=${userId}&last_sync_data_time=1597306380&device_type=0&last_deviceid=${BAND_ID}&data_json=${dataJson}`;
  ...
  return {
    ...
    message:
      ...
      response.data?.error ||
      (response.status === 200 ? "success" : response.text.slice(0, 160)),
  };
}

async function brush(user, password, step, options = {}) {
  let last = "刷步失败";
  const retries = Math.max(1, Number(options.retries) || 6);
  for (let attempt = 1; attempt <= retries; attempt += 1) {
    let dispatcher;
    try {
      dispatcher = options.dispatcherFactory ? await options.dispatcherFactory(attempt) : options.dispatcher;
      const requestOptions = { dispatcher, timeout: options.timeout };
      const signed = await login(user, password, requestOptions);
      if (!signed.loginToken) {
        last = signed.message;
        continue;
      }
      const token = await getAppToken(signed.loginToken, requestOptions);
      if (!token.appToken) {
        last = token.message;
        continue;
      }
      const changed = await changeSteps(signed.userId, token.appToken, step, requestOptions);
      if (changed.ok) return { ok: true, attempt, userId: signed.userId };
      last = changed.message;
    } catch (error) {
      last = String(error?.message || error);
    } finally {
      if (dispatcher && dispatcher !== options.dispatcher && dispatcher.close) await dispatcher.close().catch(() => {});
    }
  }
  return { ok: false, message: last };
}
module.exports = { USER_AGENT, DATA_TEMPLATE, renderData, login, getAppToken, changeSteps, brush };
