...
config = plugin.Form({
    's_xjb_config_xjb_qlname': plugin.Form.string().title('设置对接容器').default('').description('你的变量需要添加到的容器？参数用丨分割'),
    's_xjb_config_xjb_osname': plugin.Form.string().title('提交到青龙的变量名').default('').description('青龙容器内新江北的变量名'),
    's_xjb_config_notify': plugin.Form.string().title('通知渠道').default('').description('检测功能的通知渠道，多个渠道用逗号分隔'),
})
_CONFIG_FIELD_MAP = {
...
PAYMENT_CONFIG = {
    ...
    'ma_pay_switch': '2099-12-31',  # 在线处理开关
    'ma_pay_gateway': '2099-12-31',  # 从卡密系统获取支付网关
    'ma_pay_pid': '2099-12-31',  # 从卡密系统获取商户ID
    'ma_pay_key': '2099-12-31',  # 从卡密系统获取商户密钥
    'ma_pay_type': '2099-12-31',  # 从卡密系统获取支付方式
    'ma_pay_notify_url': '2099-12-31',  # 从卡密系统获取异步通知地址
    'ma_pay_return_url': '2099-12-31',  # 从卡密系统获取跳转通知地址
    'pid': '',  # 将在后面初始化
    'key': '',  # 将在后面初始化
    'gateway': '',  # 将在后面初始化
    'notify_url': '',  # 将在后面初始化
