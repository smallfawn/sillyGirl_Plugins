...
// [description: 星芽免费短剧凭证绑定、青龙同步、账号查询与清理]
// [depe: []]
...
async function showAccounts(ql, envName) {
  ...
  return s.reply([`星芽免费短剧账号：${visible.length} 个`, ...visible.map((item, index) => `${index + 1}. ${remarkOf(item) || "未备注"}${item.status ? "（已禁用）" : ""}`)].join("\n"));
}
...
function ownerKey(sender) { return "xingYaMianFeiDuanJu|" + sender.getPlatform() + ":" + sender.getUserId(); }
