...
_CONFIG_FIELD_MAP = {
    ...
    ('ch_xy_config', 'panel_config'): 'ch_xy_config_panel_config',
    ('ch_xy_config', 'panel_group'): 'ch_xy_config_panel_group',
    ('ch_xy_config', 'Qinglong'): 'ch_xy_config_Qinglong',
    ('ch_xy_config', 'osname'): 'ch_xy_config_osname',
}
...
def retry_pending_panel_sync(osname, phone_owner_map=None):
    ...
    for phone in pending_keys:
        ...
        account_vip = safe_str(sg.bucketGet(bucket='ch_xy_accountvip', key=phone))
...
class Xingya:
    def __init__(self, user, sender):
        ...
        self.sender = sender  # 获取的sg.py里面的sender类
        self.headers = {
            'X-App-Id': '7',
            'Authorization': '',
            'platform': '1',
    ...
    def zf(self,project, me_as_int):
        try:
            ...
            if not zsm and not use_ma_pay and not xycoin:
                self.sender.reply('❌ 未配置收款方式,请检查配置!')
                return False

            if int(xycoin) > 0:
                points = sg.bucketGet('ch_xy_config', "jfbl") or "dd_sign_points"
                usercoin = sg.bucketGet(points, user) or '0'
...
=================="""
                ...
                if self.yesornos():
                    try:
                        ...
                        points = sg.bucketGet('ch_xy_config', "jfbl") or "dd_sign_points"
                        sg.bucketSet(points, user, str(new_balance))
                        return True
                    except Exception as e:
...
====================""")
            ...
            if response1['code'] == 'ok':
                ...
                if response2['code'] == 'ok':
                    ...
                    sg.bucketSet("ch_xy_token", phone, token2)
                    sg.bucketSet("ch_xy_did", phone, did)
            ...
            for attempt in range(max_retry):
                try:
                    ...
                    if r['code'] == 'ok':
                        phone = r['data']['mobile']
                        token = r['data']['token']
                        sg.bucketSet("ch_xy_token", phone, token)
                        sg.bucketSet("ch_xy_did", phone, did)
                        bind_user_phone(self.user, phone)
                        success_count += 1
    ...
    def account_cx(self, phone):
        zhanghao = ''
        accountVip = safe_str(sg.bucketGet(bucket='ch_xy_accountvip', key=phone))
        if len(accountVip) == 0 or accountVip < today_time:
            accountVip = '授权已过期'
        try:
            auth = sg.bucketGet(bucket='ch_xy_token', key=phone)
            did = sg.bucketGet(bucket='ch_xy_did', key=phone)
            headers = self.headers.copy()
...
if __name__ == "__main__":
    ch_xy_osname = sg.bucketGet('ch_xy_config', 'osname') or 'ch_xy'
    ch_xy_panel_type = sg.bucketGet('ch_xy_config', 'panel_type') or ''
    ch_xy_panel_config = sg.bucketGet('ch_xy_config', 'panel_config') or ''
    ch_xy_panel_group = sg.bucketGet('ch_xy_config', 'panel_group') or ''
    ch_xy_qlname = sg.bucketGet('ch_xy_config', 'Qinglong')
    ch_xy_money = sg.bucketGet('ch_xy_config', 'money') or '10'

    dl = sg.bucketGet('ch_xy_config', 'dl') or None
    proxies = None
    is_proxy = False
    proxy_url = ""
    if dl:
        is_proxy = True
        proxy_url = dl
    xycoin = sg.bucketGet('ch_xy_config', 'jfmoney')
    ...
    uservalue = sg.bucketGet("ch_xy_phone", user)
    ...
    if '登录' in usermessage or '登陆' in usermessage:
        ...
        phone_owner_map = {}
        for key in keys:
            account_phones = split_accounts(sg.bucketGet("ch_xy_phone", key))
