...
config = plugin.Form({
    ...
    's_xyyx_qlname': plugin.Form.string().title('设置对接容器').default('').description('面板容器参数，不填则使用默认配置'),
    's_xyyx_use_daipanel': plugin.Form.boolean().title('使用呆呆面板').default(False).description('勾选使用呆呆面板，不勾选使用青龙面板'),
    's_xyyx_panel_group': plugin.Form.string().title('呆呆面板分组').default('').description('填写后新增/更新变量时同步写入 group 字段，留空则不处理'),
    's_xyyx_osname': plugin.Form.string().title('青龙变量名').default('S_XYYX').description('青龙容器内的变量名'),
    's_xyyx_notify': plugin.Form.string().title('通知渠道').default('').description('检测通知推送渠道'),
})
_CONFIG_FIELD_MAP = {
