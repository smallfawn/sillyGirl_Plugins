...
proxy_url = sg.bucketGet(bucket=BUCKET_CONFIG, key="proxy_api") or ""
...
def process_manage() -> None:
    ...
    user_coin = sg.bucketGet("dd_sign_points", userid) or "0"
    menu = (
        "=====壹品仓管理=====\n"
        f"📱 绑定账号: {len(phones)}个\n"
...
def run_tasks() -> None:
    ...
    try:
        all_user_ids = sg.bucketAllKeys(bucket=BUCKET_USER) or []
    except Exception:
        all_user_ids = []
    if not all_user_ids:
        sender.reply("❌ 暂无用户数据")
