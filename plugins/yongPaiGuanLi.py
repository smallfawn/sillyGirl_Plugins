...
# [disable: false]
# [admin: true]
# [rule: ^(甬派)(登录|登陆)$|^登(录|陆)(甬派)$|^(甬派)(查询|管理)$|^(查询|管理)(甬派)$|^甬派清理$|^甬派$|^甬派教程$|^甬派通知 ?(.*)$|^清理甬派$|^甬派广播 ?(.*)$]
# [cron: 5 10 * * *]
# [icon: https://api.iconify.design/lucide:bot.svg]
# [description: 甬派代挂提交；4.]
# [depe: ["requests"]]
...
config = form({
    'yongpai_panel_type': form.string().title('对接面板类型').default('').description('qinglong=青龙面板 daidai=呆呆面板'),
    'yongpai_yongpai_qlname': form.string().title('对接系统配置').default('').description('青龙:URL丨ID丨Secret 呆呆:URL丨Key丨Secret'),
    'yongpai_yongpai_osname': form.string().title('系统变量名').default('').description('系统容器内变量名(默认为YPCK)'),
    'yongpai_enable_proxy': form.boolean().title('启用代理').default(False).description('是否启用代理功能'),
    'yongpai_proxy_pool_url': form.string().title('代理池地址').default('').description('代理API服务地址'),
    'yongpai_enable_remark': form.boolean().title('启用备注功能').default(False).description('是否启用账号备注功能'),
})
_CONFIG_FIELD_MAP = {
    ('yongpai', 'panel_type'): 'yongpai_panel_type',
    ('yongpai', 'yongpai_qlname'): 'yongpai_yongpai_qlname',
    ('yongpai', 'yongpai_osname'): 'yongpai_yongpai_osname',
    ('yongpai', 'enable_proxy'): 'yongpai_enable_proxy',
    ('yongpai', 'proxy_pool_url'): 'yongpai_proxy_pool_url',
    ('yongpai', 'enable_remark'): 'yongpai_enable_remark',
}
...
try:
    sg.bucketSet(bucket='yongpai_sender', key=userid, value=str(senderID))
    sg.bucketSet(bucket='yongpai_imtype', key=userid, value=str(sender.getImtype()))
except:
    pass
...
def getusercontent():
    """获取插件完整配置"""
    panel_type = sg.bucketGet('yongpai', 'panel_type') or 'qinglong'
    panel_type = panel_type.lower()

    env_qlconfig = sg.bucketGet('yongpai', 'yongpai_qlname') or ''
    env_name = sg.bucketGet('yongpai', 'yongpai_osname') or 'YPCK'

    if not env_qlconfig:
        sender.reply("❌ 配置错误：请在插件配置中填写【对接系统配置】(面板信息)。")
        exit(0)

    wc_managecommand = sg.bucketGet('yongpai', 'yp_managecommand') or sg.bucketGet('yongpai', 'wc_managecommand') or '甬派管理'
    wc_querycommand = sg.bucketGet('yongpai', 'yp_querycommand') or sg.bucketGet('yongpai', 'wc_querycommand') or '甬派查询'
    wc_signcommand = sg.bucketGet('yongpai', 'yp_signcommand') or sg.bucketGet('yongpai', 'wc_signcommand') or '甬派登录'

    enable_proxy = (sg.bucketGet('yongpai', 'enable_proxy') or 'false').lower() == 'true'
    proxy_pool_url = sg.bucketGet('yongpai', 'proxy_pool_url') or ''
    points_bucket = sg.bucketGet('yongpai', 'points_bucket') or 'dd_sign_points'
    enable_remark = (sg.bucketGet('yongpai', 'enable_remark') or 'false').lower() == 'true'

    randommanagecommand = wc_managecommand
    randomquerycommand = wc_querycommand
    randomsigncommand = wc_signcommand

    zsVipmoney = Decimal(sg.bucketGet('yongpai', 'zsVipmoney') or '0')
    zscoin = int(sg.bucketGet('yongpai', 'zscoin') or '0')
    reminder_days = int(sg.bucketGet('yongpai', 'reminder_days') or '2')

    enable_zsm = (sg.bucketGet('yongpai', 'enable_zsm') or 'false').lower() == 'true'
    zsm = sg.bucketGet('yongpai', 'zsm') or ''
...
def get_owner_user_id(account, fallback_userid=None):
    ...
    try:
        ...
        pass
    try:
        for owner in sg.bucketAllKeys(bucket='yongpai_user'):
            try:
...
def send_daily_admin_report(report_data, force_send=False, notify_status=False):
    ...
    if not force_send and sg.bucketGet('yongpai_runtime', report_key):
        if notify_status:
    ...
    if framework_sent:
        try:
            sg.bucketSet('yongpai_runtime', report_key, "framework")
        except Exception:
            pass
        if notify_status:
...
def send_user_notice(user_id, msg, title="甬派通知"):
    try:
        imtype = sg.bucketGet(bucket='yongpai_imtype', key=str(user_id)) or sender.getImtype()
    except:
    ...
    targets = []
    try:
        saved_sender = sg.bucketGet(bucket='yongpai_sender', key=str(user_id))
        if saved_sender:
...
class RemarkManager:
    ...
    def get_account_remark(user_id, account_id):
        try:
            remark_data = sg.bucketGet(bucket='yongpai_remarks', key=f'{user_id}_{account_id}')
    ...
    def set_account_remark(user_id, account_id, remark):
        try:
            ...
            if remark_clean:
                sg.bucketSet(bucket='yongpai_remarks', key=f'{user_id}_{account_id}', value=remark_clean)
                return remark_clean
            return ""
        except: return ""
    ...
    def delete_account_remark(user_id, account_id):
        try:
            sg.bucketDel(bucket='yongpai_remarks', key=f'{user_id}_{account_id}')
            return True
        except: return False
...
class AccountManager:
    ...
    def get_accounts(user_id):
        try:
            value = sg.bucketGet(bucket='yongpai_user', key=str(user_id))
            if not value: return []
    ...
    def add_account(user_id, account):
        try:
            ...
            if account not in accounts:
                ...
                sg.bucketSet(bucket='yongpai_user', key=str(user_id), value=str(accounts))
                return True
            return False
        except: return False
        ...
        try:
            ...
            if account in accounts:
                ...
                if accounts:
                    ...
                    sg.bucketDel(bucket='yongpai_user', key=str(user_id))
                return True
            return False
        except: return False
    ...
    def update_account_token(account, token):
        try:
            ...
            sg.bucketSet(bucket='yongpai_token', key=str(account), value=encrypted_token)
            return True
        except: return False
    ...
    def get_token(account):
        try:
            enc = sg.bucketGet(bucket='yongpai_token', key=str(account))
    ...
    def get_all_users():
        try:
            users = sg.bucketAllKeys(bucket='yongpai_user')
            user_list = []
            for user in users:
    ...
    def migrate_account(user_id, old_account, new_account, new_token, remark=""):
        try:
            ...
            old_bind_date = sg.bucketGet(bucket='yongpai_bind_date', key=old_account)
            if old_bind_date and not sg.bucketGet(bucket='yongpai_bind_date', key=new_account):
                sg.bucketSet(bucket='yongpai_bind_date', key=new_account, value=old_bind_date)
            ...
            sg.bucketSet(bucket='yongpai_user', key=str(user_id), value=str(new_accounts))

            AccountManager.update_account_token(new_account, new_token)
            try: sg.bucketDel(bucket='yongpai_token', key=old_account)
            except: pass
            try:
                pass
            except: pass
...
def process_account_binding(full_token, unique_id, nickname, remark="", aliases=None, acc_type="", legacy_key="", silent=False):
    try:
        ...
        if is_new:
            try: sg.bucketSet(bucket='yongpai_bind_date', key=account, value=str(datetime.now().date()))
            except: pass
        ...
        if choice == '1':
            ...
            if process_payment(months, accountVip, token, account, remark):
                try:
                    ...
                    for d in range(config['reminder_days'] + 1):
                        ...
                        try: sg.bucketDel('yongpai_remind_log', remind_key)
                        except: pass
            ...
            if get_user_input() == 'y':
                try:
                    ...
                    try: sg.bucketDel(bucket='yongpai_token', key=account)
                    except: pass
                    try:
                        pass
                    except: pass
            ...
            for account_item in context["accounts"]:
                ...
                if days_diff >= 0:
                    ...
                    if not is_ck_valid:
                        ...
                        has_ck_reminded = sg.bucketGet('yongpai_remind_log', ck_remind_key)
...
=================="""
                            if safe_send_message(user, msg, f"配置失效提醒 {user}-{account}"):
                                try: sg.bucketSet('yongpai_remind_log', ck_remind_key, "1")
                                except: pass
                                ck_expired_count += 1

                    if is_ck_valid and 0 <= days_diff <= reminder_days_cfg:
                        remind_key = f"{user}_{account}_{today_date}"
                        has_reminded = sg.bucketGet('yongpai_remind_log', remind_key)
...
=================="""
                            if safe_send_message(user, msg, f"到期提醒 {user}-{account}"):
                                try: sg.bucketSet('yongpai_remind_log', remind_key, "1")
                                except: pass
                                reminded_count += 1
                    continue
...
=================="""
            ...
            if user_has_change:
                if valid_accounts:
                    try: sg.bucketSet(bucket='yongpai_user', key=str(user), value=str(valid_accounts))
                    except: pass
                else:
                    try: sg.bucketDel(bucket='yongpai_user', key=str(user))
                    except: pass
