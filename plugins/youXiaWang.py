...
config = form({
    'mrconli_youxia_ql_config': form.string().title('对接青龙').default('').description('|'),
    'mrconli_youxia_var_name': form.string().title('环境变量名').default('').description('青龙容器内的变量名，默认为：m_yxw'),
    'mrconli_youxia_is_proxy': form.boolean().title('是否启用代理').default(False).description('true/false'),
    'mrconli_youxia_proxy_pool': form.string().title('代理池地址').default('').description('代理API服务地址'),
})
_CONFIG_FIELD_MAP = {
    ('mrconli', 'youxia.ql_config'): 'mrconli_youxia_ql_config',
    ('mrconli', 'youxia.var_name'): 'mrconli_youxia_var_name',
    ('mrconli', 'youxia.is_proxy'): 'mrconli_youxia_is_proxy',
    ('mrconli', 'youxia.proxy_pool'): 'mrconli_youxia_proxy_pool',
}
...
uservalue = sg.bucketGet(bucket='mrconli.youxia.user', key=userid)
today_date = datetime.now().date()
today_time = str(today_date)


MAX_RETRIES = 10  # 最大重试次数
IS_PROXY = sg.bucketGet('mrconli.youxia', 'is_proxy')  # 是否启用代理True
PROXY_API = sg.bucketGet('mrconli.youxia', 'proxy_pool')
...
def batch_login():
    ...
    for index, account in enumerate(accounts, 1):
        try:
            ...
            if phone:
                success_count += 1
                sg.bucketSet('mrconli.youxia.token', phone, token)
                sg.bucketSet('mrconli.youxia.bind', phone, account)
                current_accounts = _sg_literal(sg.bucketGet('mrconli.youxia.user', userid) or '[]')
                if phone not in current_accounts:
                    add_count += 1
                    status = "✅ 登录成功"
                    current_accounts.append(phone)
                    sg.bucketSet('mrconli.youxia.user', userid, json.dumps(current_accounts, ensure_ascii=False))
                else:
                    update_count += 1
                    status = "✅ 更新成功"
...
def query():
    ...
    for account in target_accounts:
        try:
            accountVip = '2099-12-31'
            bind = sg.bucketGet('mrconli.youxia.bind', account)
...
def get_config():
    """获取插件配置"""
    try:
        sg.bucketGet('mrconli.youxia', 'coin_bucket') or 'dd_sign_points'
        var_name = sg.bucketGet('mrconli.youxia', 'var_name') or "m_yxw"
        if not var_name:
            print("未配置变量名，使用默认值: m_yxw")
            var_name = 'm_yxw'
            sg.bucketSet('mrconli.youxia', 'var_name', var_name)
        ql_config = sg.bucketGet('mrconli.youxia', 'ql_config')
        if not ql_config:
        ...
        if len(ql_params) == 3:
            ...
            print("青龙配置不完整，请检查配置")
        manage_cmd = sg.bucketGet('mrconli.youxia', 'manage_cmd') or '游侠管理'
        query_cmd = sg.bucketGet('mrconli.youxia', 'query_cmd') or '游侠查询'
        login_cmd = sg.bucketGet('mrconli.youxia', 'login_cmd') or '游侠登录'
        try:
            price = Decimal(sg.bucketGet('mrconli.youxia', 'price') or '1')
            if price < 0:
                raise ValueError("价格不能为负数")
        except (ValueError, decimal.InvalidOperation):
            print("价格配置无效，使用默认值: 1")
            price = Decimal('1')
            sg.bucketSet('mrconli.youxia', 'price', '1')
        try:
            coin_price = int(sg.bucketGet('mrconli.youxia', 'coin') or '0')
            if coin_price < 0:
                raise ValueError("积分不能为负数")
        except ValueError:
            print("积分配置无效，使用默认值: 0")
            coin_price = 0
            sg.bucketSet('mrconli.youxia', 'coin', '0')
        try:
            show_records = int(sg.bucketGet('mrconli.youxia', 'show_records') or '3')
            if show_records < 1:
                raise ValueError("显示记录数不能小于1")
        except ValueError:
            print("显示记录数配置无效，使用默认值: 3")
            show_records = 3
            sg.bucketSet('mrconli.youxia', 'show_records', '3')
    ...
    try:
        ...
        for attempt in range(max_retries):
            ...
            if response.status_code == 200:
                ...
                sg.bucketSet('mrconli.youxia.env_id', account, json.dumps(new_ids))
                return True
...
账号列表:"""
    for i, account in enumerate(accounts, 1):
        token = sg.bucketGet('mrconli.youxia.token', account)
        auth = '2099-12-31'
    ...
    try:
        if choice == '01':
            ...
            sg.bucketSet('mrconli.youxia.user', userid, '[]')
            ...
            if not days:
                ...
                sender.reply("✅ 已取消授权")
                return
            coin_bucket = sg.bucketGet('mrconli.youxia', 'coin_bucket') or 'dd_sign_points'
            coin_price = int(sg.bucketGet('mrconli.youxia', 'coin') or '0')  # 确保获取最新积分价格
                ...
                if pay_choice == '1':
                    ...
                    if process_payment(amount, days):
                        ...
                        for account in accounts:
                            ...
                            True
                            token = sg.bucketGet('mrconli.youxia.bind', account)
                            username = account
...
====================""")
                ...
                for account in accounts:
                    env_id_str = sg.bucketGet('mrconli.youxia.env_id', account)
                    if env_id_str:
...
def show_account_menu(account):
    """显示账号操作菜单"""
    sg.bucketGet('mrconli.youxia.token', account)
    auth = '2099-12-31'
    username = f"{account}"
    auth_status = "✅ 已授权" if auth and auth > today else "❌ 未授权"
...
def cron_task():
    ...
    if imtype != 'fake':
        return
    try:
        users = sg.bucketAllKeys('mrconli.youxia.user')
        for user in users:
            accounts = _sg_literal(sg.bucketGet('mrconli.youxia.user', user) or '[]')
            for account in accounts:
                try:
                    auth = '2099-12-31'
                    bind = sg.bucketGet('mrconli.youxia.bind', account)
...
def log_operation(operation, user, account, status, message=''):
    ...
    try:
        ...
        logs = _sg_literal(sg.bucketGet('mrconli.youxia.logs', 'operations') or '[]')
        logs.append(log)
        if len(logs) > 1000:  # 只保留最近1000条
            logs = logs[-1000:]
        sg.bucketSet('mrconli.youxia.logs', 'operations', str(logs))
    except Exception as e:
    ...
    for user in users:
        ...
        for account in accounts:
            ...
            try:
                ...
                env_ids_str = sg.bucketGet('mrconli.youxia.env_id', account)
                if env_ids_str:
...
回复"q"退出""")
    ...
    if not user_id or user_id == 'q':
        return
    accounts = _sg_literal(sg.bucketGet('mrconli.youxia.user', user_id) or '[]')
    if not accounts:
...
def delete_account(account):
    ...
    try:
        ...
        sg.bucketDel('mrconli.youxia.token', account)
        True
        sg.bucketDel('mrconli.youxia.env_id', account)
        sg.bucketDel('mrconli.youxia.bind', account)
        ...
        if account in accounts:
            ...
            try:
                sg.bucketSet('mrconli.youxia.user', userid, json.dumps(accounts, ensure_ascii=False))
            except Exception as e:
