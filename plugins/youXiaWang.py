...
config = plugin.Form({
    ...
    'mrconli_youxia_ql_config': plugin.Form.string().title('对接青龙').default('').description('|'),
    'mrconli_youxia_var_name': plugin.Form.string().title('环境变量名').default('').description('青龙容器内的变量名，默认为：m_yxw'),
    'mrconli_youxia_is_proxy': plugin.Form.boolean().title('是否启用代理').default(False).description('true/false'),
    'mrconli_youxia_proxy_pool': plugin.Form.string().title('代理池地址').default('').description('代理API服务地址'),
})
_CONFIG_FIELD_MAP = {
