...
config = plugin.Form({
    ...
    'dd_zm_dd_zm_qlname': plugin.Form.string().title('设置对接系统').default('').description('你的变量需要添加到的系统容器？参数用丨分割'),
    'dd_zm_dd_zm_osname': plugin.Form.string().title('提交到系统的变量名').default('').description('系统容器内战马的变量名(默认为zmnlyl)'),
    'dd_zm_enable_proxy': plugin.Form.boolean().title('是否启用代理').default(False).description('是否启用代理功能'),
    'dd_zm_proxy_pool_url': plugin.Form.string().title('代理池地址').default('').description('代理API服务地址'),
    'dd_zm_enable_remark': plugin.Form.boolean().title('启用备注功能').default(False).description('是否启用账号备注功能'),
})
_CONFIG_FIELD_MAP = {
